...
{{-- DESKTOP: hover-expand tanpa toggle --}}
<aside
  class="group/sidebar fixed inset-y-0 left-0 z-30 hidden lg:flex lg:flex-col bg-base-100 border-r border-base-300/50 transition-[width] duration-300 ease-in-out contain-paint"
  :class="$store.ui.sidebarHover ? 'w-64' : 'w-20'"
  @mouseenter="$store.ui.sidebarHover = true"
  @mouseleave="$store.ui.sidebarHover = false"
>
  <div class="flex h-20 items-center border-b border-base-300/50" :class="$store.ui.sidebarHover ? 'px-4 justify-start' : 'px-2 justify-center'">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
      <div class="h-10">
        <x-application-logo class="shrink-0"/>
      </div>
      <div x-show="$store.ui.sidebarHover" x-transition class="origin-left self-center">
        <span class="badge badge-primary badge-outline">growth</span>
      </div>
    </a>
  </div>
...
>
  <div class="flex h-20 items-center border-b border-base-300/50 px-4">
    ...
    <button class="ml-auto btn btn-ghost btn-circle" @click="$store.ui.isMobileMenuOpen = false" aria-label="Tutup menu">✕</button>
  </div>
