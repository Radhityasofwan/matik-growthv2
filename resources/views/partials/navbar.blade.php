<div class="sticky top-0 z-30 p-4 pb-0 sm:p-6 sm:pb-0">
  <header data-aos="fade-down" class="navbar rounded-box border border-base-300/20 bg-base-100/80 shadow md:shadow-md backdrop-blur supports-[backdrop-filter]:backdrop-blur-lg" role="banner">
    <div class="navbar-start gap-2">
      <button
        type="button"
        @click="$store.ui.isMobileMenuOpen = true"
        class="btn btn-ghost btn-circle lg:hidden"
        :aria-expanded="$store.ui.isMobileMenuOpen.toString()"
        aria-label="Buka menu"
      >
        <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg>
      </button>
      <div class="hidden lg:flex items-center gap-2">
        <span x-text="greet.icon" class="text-xl"></span>
        <div>
            <span class="font-semibold text-base-content/80"><span x-text="greet.text"></span>, {{ Str::of(Auth::user()->name)->explode(' ')->first() }}!</span>
            <p class="text-xs text-base-content/60">Welcome back, ayo buat hari ini produktif.</p>
        </div>
      </div>
    </div>
    ...
    <div class="navbar-end gap-1">
      <button
        type="button"
        @click="$store.ui.isSearchOpen = true; $nextTick(() => $refs.searchInput?.focus())"
      ...
      <div class="dropdown dropdown-end" x-data="notifBell($el)" x-init="init()" data-unread="{{ (int) ($unreadNotificationsCount ?? 0) }}" data-preview='@json($notificationsPreview ?? [])' data-feed="{{ route('notifications.feed') }}" data-read-base="/notifications" data-read-all="{{ route('notifications.read_all') }}">
        <label tabindex="0" class="btn btn-ghost btn-circle" @click="open = !open" aria-label="Notifikasi">
          <div class="indicator">
        ...
        <div x-show="open" x-transition tabindex="0" class="dropdown-content z-[1] mt-4 w-80 sm:w-96 bg-base-100 shadow-lg rounded-box border border-base-300/50" @click.outside="open=false">
          ...
          <div class="p-2 border-t border-base-300/50"><a href="{{ route('notifications.index') }}" class="btn btn-ghost btn-sm w-full" @click="open=false">Lihat Semua Notifikasi</a></div>
        </div>
      </div>
        ...
        <label tabindex="0" class="btn btn-ghost flex items-center gap-2">
          <div class="avatar">
            <div class="w-8 rounded-full ring ring-primary/50 ring-offset-base-100 ring-offset-2">
                @if (Auth::user()->avatar)
                    <img loading="lazy" src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="Avatar pengguna"/>
                @else
                    <img loading="lazy" src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=3B82F6&color=fff&size=128" alt="Avatar pengguna"/>
                @endif
            </div>
          </div>
