{{-- resources/views/sales/leads/index.blade.php --}}
@extends('layouts.app')
@section('title', 'Leads - Matik Growth Hub')

@push('styles')
<style>
/* Pastikan dropdown/flyout tidak ketutup container */
.modal .modal-box { overflow: visible; }
.dropdown .dropdown-content { z-index: 60; }
select.select { position: relative; z-index: 30; }

/* Perbaiki clipping pada container tabel saat dropdown terbuka di dalamnya */
.table-container, .card-body, .card { overflow: visible; }

/* Tinggikan z-index flyout "Opsi" di header */
.flyout-high { position: relative; z-index: 70; }
</style>
@endpush
...
<div class="container mx-auto py-6">
    ...
    @if ($errors->any())
        <div class="alert alert-error shadow-lg mb-6" data-aos="fade-down">
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m7 10a9 9 0 11-18 0 9 0 0118 0z" /></svg>
                <span>
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4" data-aos="fade-down">
        ...
        <div class="flex flex-wrap items-center gap-2 flyout-high">
            ...
            <div class="flex flex-wrap items-center gap-2 relative">
                {{-- Opsi: tombol di kiri + flyout ke kiri --}}
                <div x-data="{ open:false }" class="relative">
                    <button type="button"
                            ...
                            :aria-expanded="open"
                            aria-haspopup="true"
                            class="btn btn-outline btn-sm gap-2 text-base-content">
                        {{-- ikon kebab --}}
                        <svg class="h-4 w-4 opacity-70" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                    ...
                    {{-- flyout kolom di kiri tombol --}}
                    <div x-cloak
                        x-show="open"
                        x-transition.opacity
                        x-transition.scale.origin-right
                        @click.outside="open=false"
                        class="absolute right-full mr-2 top-1/2 -translate-y-1/2 z-50
                               w-64 sm:w-72 max-w-[calc(100vw-2rem)]">
                        <div class="card bg-base-100 text-base-content border border-base-300/60 shadow-2xl rounded-xl">
                            <ul class="menu menu-sm p-2">
                                <li>
                                    <a href="#import_modal" @click="open=false">
                                        <svg class="h-4 w-4 opacity-70" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                            <path d="M12 3v12m0 0l-4-4m4 4l4-4M4 21h16"></path>
                                        </svg>
                                        Import dari File
                                    </a>
                                </li>
                                <li>
                                    <a href="{{ route('lead-follow-up-rules.index') }}" @click="open=false">
                                        <svg class="h-4 w-4 opacity-70" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                                            <path d="M12 22a2 2 0 01-2-2h4a2 2 0 01-2 2zM18 16V11a6 6 0 10-12 0v5l-2 2h16l-2-2z"></path>
                                        </svg>
    ...
    @php
        ...
        $statusBadge = [
            ...
            'nonactive' => 'badge-neutral',
            'converted' => 'badge-primary',
            'churn'     => 'badge-error',
        ];
    @endphp

    {{-- Kolom per kategori chat --}}
                ...
                <div class="overflow-x-auto overflow-y-visible table-container">
                    ...
                    <table class="table table-zebra w-full">
                        <thead class="bg-base-200 text-base-content">
                            <tr>
                                <th class="p-3">
                                    <label><input type="checkbox" class="checkbox checkbox-xs group-select-all"></label>
                                </th>
                                <th>Nama</th>
                                <th>Status</th>
                        ...
                        <tbody>
                            ...
                            <tr id="lead-row-{{ $lead->id }}" class="hover"
                                ...
                                <td class="p-3">
                                    <label>
                                        <input type="checkbox"
                                               class="checkbox checkbox-xs lead-checkbox"
                                               value="{{ $lead->id }}"
                                               data-phone="{{ preg_replace('/\D+/', '', (string)($lead->phone ?? '')) }}"
                                ...
                                <td>
                                    <a href="{{ route('leads.show', $lead) }}" class="link link-hover link-primary font-medium">
                                        {{ $lead->name ?? '-' }}
                                    </a>
                                    <div class="text-xs text-base-content/60">{{ $lead->phone ?? '-' }}</div>
                                </td>
                                <td>
                                    @php $badge = $statusBadge[$lead->status] ?? 'badge-outline'; @endphp
                                    <span class="badge {{ $badge }}">{{ $statusLabel[$lead->status] ?? $lead->status }}</span>
                                </td>
                                <td class="text-center">
                                    <div class="dropdown dropdown-end">
                                        <label tabindex="0" class="btn btn-ghost btn-xs">opsi</label>
                                        <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-44 z-50 border border-base-300/50">
                                            @if($lead->phone)
...
{{-- =================== MODALS (di-push keluar <main>) =================== --}}
@push('modals')
    {{-- Create Lead --}}
    <div id="create_lead_modal" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box w-11/12 max-w-2xl max-h-[80vh] overflow-visible">
    ...
    <div id="edit_lead_modal_{{ $lead->id }}" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box w-11/12 max-w-2xl max-h-[80vh] overflow-visible">
    ...
    {{-- Import Modal --}}
    <div id="import_modal" class="modal modal-bottom sm:modal-middle">
    ...
    <div id="whatsapp_modal" class="modal modal-bottom sm:modal-middle">
                ...
                <div class="relative z-40">
                    ...
                    <select id="wa-template-selector" class="select select-bordered w-full">
                        ...
                        @foreach ($whatsappTemplates as $template)
                            {{-- ⛏️ data-body-b64 untuk aman terhadap newline & kutip --}}
                            <option value="{{ $template->id }}" data-body-b64="{{ base64_encode($template->body) }}">{{ $template->name }}</option>
                        @endforeach
                    </select>
                </div>
    ...
    <div id="bulk_whatsapp_modal" class="modal modal-bottom sm:modal-middle">
        <div class="modal-box w-11/12 max-w-2xl overflow-visible">
            <a href="#" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</a>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const fromB64 = s => {
        try { return atob((s||'').trim()); } catch(e) { return ''; }
    };
    ...
    if (waTplSel) waTplSel.addEventListener('change', function(){
        const bodyB64 = this.selectedOptions?.[0]?.dataset?.bodyB64 || this.selectedOptions?.[0]?.dataset?.bodyb64 || '';
        const body = fromB64(bodyB64);
    ...
    if (bulkTplSel) bulkTplSel.addEventListener('change', function(){
        const bodyB64 = this.selectedOptions?.[0]?.dataset?.bodyB64 || this.selectedOptions?.[0]?.dataset?.bodyb64 || '';
        const body = fromB64(bodyB64);
        if(bulkPrev) bulkPrev.value = body;
    ...
    // ===== Manual WA link + penanda chat_count =====
