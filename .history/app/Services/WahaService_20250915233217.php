...
class WahaService
{
    ...
    // 👇 Accept: prioritas image, lalu JSON
    ...
    "/api/{$this->e($session)}/auth/qr",      // NEW
    "/api/{$this->e($session)}/auth/qr.png",  // NEW
...
public function qrImageBinary(WahaSender|string $senderOrSession): array
{
    ...
    // Banyak build gonta-ganti path; coba semuanya.
   $paths = [
    "/api/sessions/{$this->e($session)}/qr",
    "/api/sessions/{$this->e($session)}/qr.png",
    "/api/sessions/{$this->e($session)}/qr-image",
    ...
    foreach ($paths as $p) {
        ...
        try {
            ...
            if ($status >= 200 && $status < 300) {
                // 1) Kalau server sudah kirim image binary → langsung stream
                if (str_starts_with($ctype, 'image/')) {
                    return ['success'=>true, 'ctype'=>$ctype ?: 'image/png', 'body'=>$body];
                }

                // 2) Kadang content-type kosong tapi isinya binary (octet-stream)
                ...
                // 3) Coba parse JSON berisi base64/dataURI
                $js = null;
    ...
    // 5) Terakhir: probe status; bisa jadi state bukan SCAN_QR_CODE lagi
    $probe = $this->getSessionInfo($session);
    if ($probe['success']) {
        $norm = $this->normalizeSession($probe['raw']);
        if ($norm['connected'] === true) {
            // Sudah tersambung; QR memang tidak ada lagi
...
/** Ekstrak pairing code dari berbagai bentuk JSON */
    ...
    foreach ($paths as $p) {
        ...
        try {
            ...
            if ($res->successful()) {
                ...
                if (is_array($number) && isset($number['user'])) $number = $number['user']; // beberapa build
 ...
 */
public function sendMessage(WahaSender|string $senderOrSession, string $phone, string $text): array
{
    $session = $this->asSession($senderOrSession);

    // Target chatId variants
    $num      = $this->formatPhone($phone);
    $chatIdCu = preg_match('~@~', $num) ? $num : ($num.'@c.us');
    $chatIdSw = preg_match('~@~', $num) ? $num : ($num.'@s.whatsapp.net');

    // Beberapa server minta 'session' di path, sebagian lain di query/body.
    $urls = array_merge(
        $this->buildSendUrlsWithPath($session),
        $this->buildSendUrlsQuery($session) // varian ?session=
    );

    // Variasi payload (urutkan dari yang paling umum)
    $payloads = [
        ['chatId' => $chatIdCu, 'message' => $text],
        ['chatId' => $chatIdCu, 'text'    => $text],
        ['chatId' => $chatIdSw, 'message' => $text],
        ['to'     => $chatIdCu, 'text'    => $text],
        ['to'     => $num,      'text'    => $text],
        ['phone'  => $num,      'body'    => $text],
        // sebagian build baca "messageText"
        ['to'     => $chatIdCu, 'messageText' => $text],
    ];
    ...
    foreach ($urls as $url) {
        foreach ($payloads as $pl) {
            try {
                $res  = $this->clientJson()->post($url, $pl);
                $json = $res->json() ?? [];

                // 2xx dengan sinyal sukses → selesai
                if ($res->successful() && $this->isSuccessful($json, $res->status())) {
                    return ['success'=>true, 'raw'=>$json, 'error'=>null];
                }

                // simpan error ringan & lanjut ke kombinasi lain
                $bodySnippet = trim(mb_substr((string)$res->body(), 0, 200));
                $lastErr = "HTTP ".$res->status().($bodySnippet ? " BODY: ".$bodySnippet : '');
            } catch (\Throwable $e) {
                $lastErr = "EXC ".$e->getMessage();
                // lanjut coba path/payload lain
            }
        }
    }

    return ['success'=>false, 'raw'=>null, 'error'=>$lastErr ?: 'All WAHA send paths failed'];
}
...
{
    // 204 No Content / 202 Accepted sering berarti diterima/queued
...
protected function formatPhone(string $p): string
{
    ...
    // opsional: auto-convert 08xxxx → 628xxxx (Indonesia)
...
protected function formatChatId(string $p): string
{
    ...
    return preg_match('~@~', $num) ? $num : ($num.'@c.us');
}
}
