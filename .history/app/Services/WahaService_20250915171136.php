...
class WahaService
{
    ...
    public function sessionStatus(WahaSender|string $senderOrSession): array
    {
        ...
        $try = [
            "/api/sessions/{$this->seg($session)}/status",
            "/api/session/{$this->seg($session)}/status",
            "/sessions/{$this->seg($session)}/status",
            "/session/{$this->seg($session)}/status",

            "/api/sessions/{$this->seg($session)}/state",
            "/api/session/{$this->seg($session)}/state",
            "/sessions/{$this->seg($session)}/state",
            "/session/{$this->seg($session)}/state",
    ...
    public function sessionStart(WahaSender|string $senderOrSession): array
    {
        ...
        // beberapa WAHA mensyaratkan key yang berbeda
        $payloads = [
            ['session'   => $session],
            ['sessionId' => $session],
            ['name'      => $session],
        ...
        $paths = [
            ...
            "/api/sessions/{$this->seg($session)}/start",
            "/api/session/{$this->seg($session)}/start",
            "/sessions/{$this->seg($session)}/start",
            "/session/{$this->seg($session)}/start",
