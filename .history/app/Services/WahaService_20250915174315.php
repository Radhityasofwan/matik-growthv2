...
class WahaService
{
    ...
    public function qrStart(WahaSender|string $senderOrSession): array
    {
        ...
        if ($start['qr'] || $start['connected'] === true) return $start;

        // Poll up-to 10x (±10s) cari QR / CONNECTED
        $last = $start;
        for ($i=0; $i<10; $i++) {
            sleep(1);
            ...
            // Jika FAILED → coba restart sekali lalu lanjut polling
            ...
            if ($st['qr'] || $st['connected'] === true) return $st;
        }
        return $last; // kembalikan info terakhir (include error jika ada)
    }
