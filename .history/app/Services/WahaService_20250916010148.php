...
public function sendMessage(WahaSender $sender, string $recipient, string $message): ?array
{
    ...
    // Beberapa build WAHA beda-beda: path & key payload
    $paths = [
        '/api/sendText',          // ✅ sesuai Swagger kamu
    ...
    $payloads = [
        ['session' => $session, 'chatId'  => $chatId, 'text'    => $message], // standar
        ['session' => $session, 'chatId'  => $chatId, 'message' => $message], // key: message
        ['session' => $session, 'receiver'=> $chatId, 'text'    => $message], // key: receiver
    ];
    ...
    foreach ($paths as $p) {
        ...
        foreach ($payloads as $body) {
            try {
                ...
                if ($res->successful()) {
                    // beberapa WAHA tidak kirim body; anggap sukses
