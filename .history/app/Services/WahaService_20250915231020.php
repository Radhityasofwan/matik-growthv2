...
/** Ekstrak pairing code dari berbagai bentuk JSON */
    ...
    /**
     * Kirim pesan teks sederhana. Mencoba beberapa path WAHA yang umum.
     * @return array Normalized: ['success'=>bool, 'raw'=>mixed, 'error'=>?string]
     */
    public function sendMessage(WahaSender|string $senderOrSession, string $phone, string $text): array
    {
        $session = $this->asSession($senderOrSession);
        $payloads = [
            // Beberapa build butuh {"chatId": "628xxx@c.us", "message": "..."}
            function() use ($phone, $text) {
                return ['chatId' => $this->formatChatId($phone), 'message' => $text];
            },
            // Variasi lain
            function() use ($phone, $text) {
                return ['to' => $this->formatChatId($phone), 'text' => $text];
            },
            function() use ($phone, $text) {
                return ['to' => $this->formatPhone($phone), 'text' => $text];
            },
        ];

        $paths = [
            "/api/sessions/{$this->e($session)}/messages/send",
            "/api/sessions/{$this->e($session)}/messages/text",
            "/api/{$this->e($session)}/messages/send",
            "/api/{$this->e($session)}/sendMessage",
            "/api/{$this->e($session)}/send/text",
        ];

        foreach ($paths as $p) {
            $url = $this->url($p);
            foreach ($payloads as $makePayload) {
                try {
                    $res = $this->clientJson()->post($url, $makePayload());
                    $json = $res->json() ?? [];
                    if ($res->successful()) {
                        return ['success' => $this->isSuccessful($json), 'raw' => $json, 'error' => null];
                    }
                    // Jika 4xx/5xx, simpan error tapi coba path lain
                    $err = "HTTP ".$res->status();
                    $snippet = trim(mb_substr((string)$res->body(), 0, 200));
                    if ($snippet) $err .= " BODY: ".$snippet;
                } catch (\Throwable $e) {
                    $err = "EXC ".$e->getMessage();
                }
            }
        }

        return ['success'=>false, 'raw'=>null, 'error'=>'All send paths failed'];
    }

    /** Normalisasi penanda sukses di berbagai bentuk respons WAHA */
    public function isSuccessful(array $resp): bool
    {
        // Sering ada: success: true | sent: true | messageId ada | status == 'ok'
        $candidates = [
            fn($r) => isset($r['success']) && $r['success'] === true,
            fn($r) => isset($r['sent']) && (bool)$r['sent'] === true,
            fn($r) => isset($r['status']) && in_array(strtolower((string)$r['status']), ['ok','success','sent'], true),
    ...
    protected function formatChatId(string $p): string
    {
        ...
        // Banyak server WAHA menggunakan @c.us
