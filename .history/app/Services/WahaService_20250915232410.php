    ...
    public function sendMessage(WahaSender|string $senderOrSession, string $phone, string $text): array
{
    ...
    $targets = [
        $this->formatChatId($phone),
        $this->formatPhone($phone),
    ];

    $payloads = [];
    foreach ($targets as $to) {
        $payloads[] = ['chatId' => $to, 'message' => $text];
        $payloads[] = ['to' => $to, 'text' => $text];
        $payloads[] = ['phone' => $to, 'body' => $text];
    }
    ...
    foreach ($paths as $p) {
        ...
        foreach ($payloads as $pl) {
            try {
                ...
                $json = $res->json() ?? [];
                if ($res->successful() && $this->isSuccessful($json)) {
    ...
    return ['success'=>false, 'raw'=>null, 'error'=>'All WAHA send paths failed'];
}
}
