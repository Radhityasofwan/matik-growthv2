...
public function qrImageBinary(WahaSender|string $senderOrSession): array
{
    ...
    foreach ($paths as $p) {
        ...
        try {
            ...
            if ($res->successful()) {
                // 1) langsung image binary
                if (str_starts_with($ctype, 'image/')) {
                    return ['success'=>true, 'ctype'=>$ctype, 'body'=>$body];
                }
                // 2) JSON dengan qr base64
                $js = null; try { $js = $res->json(); } catch (\Throwable $e) {}
                if (is_array($js)) {
                    // reuse extractor
                    $qrDataUri = $this->readQr($js) ?: $this->findBase64ImageDeep($js);
                    if (is_string($qrDataUri) && preg_match('~^data:image/[^;]+;base64,(.+)$~', $qrDataUri, $m)) {
