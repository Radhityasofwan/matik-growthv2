...
class SendLeadFollowUpJob implements ShouldQueue
{
    ...
    protected function processRule(LeadFollowUpRule $rule): void
    {
        ...
        foreach ($eligible as $lead) {
            try {
                ...
                // Notifikasi ke owner (jika ada wa_number)
    ...
    /**
     * Ambil leads eligible untuk suatu rule.
     * Pakai alias meta_* agar tidak bentrok dengan kolom asli.
     */
    ...
    /**
     * Kirim via WAHA, kompatibel dengan berbagai signature controller:
     * - sendMessage(Request $request)
     * - sendMessage(Request $request, WahaService $svc)
     * - dst (parameter class lain akan di-resolve dari container)
     */
    protected function sendViaWaha(int $senderId, string $recipient, string $message): array
    {
        ...
        // Bangun argumen dinamically berdasar type-hint
        $args = [];
        try {
            $ref = new \ReflectionMethod($controller, 'sendMessage');
            foreach ($ref->getParameters() as $idx => $param) {
                $type = $param->getType();
                $className = ($type && !$type->isBuiltin()) ? $type->getName() : null;

                // Param pertama biasanya Request
                if ($className === Request::class || $param->getName() === 'request') {
                    $args[] = $req;
                    continue;
                }

                // Kalau ada type class lain → resolve dari container
                if ($className && class_exists($className)) {
                    $args[] = app()->make($className);
                    continue;
                }

                // Fallback: kalau opsional pakai default; kalau tidak, lempar
                if ($param->isDefaultValueAvailable()) {
                    $args[] = $param->getDefaultValue();
                } else {
                    throw new \RuntimeException("Tidak bisa membangun argumen #{$idx} untuk sendMessage()");
                }
            }
        } catch (\Throwable $e) {
            // Jika refleksi gagal, fallback ke 1 argumen (Request)
            $args = [$req];
        }

        $resp = $controller->sendMessage(...$args);
