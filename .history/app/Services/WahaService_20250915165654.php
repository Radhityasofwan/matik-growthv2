...
class WahaService
{
    ...
    public function qrStatus(WahaSender|string $senderOrSession): array
    {
        ...
        $try = [
            ...
            "/sessions/{$session}/qr",
            "/session/{$session}/qr",
            "/api/qr?session={$this->e($session)}",      // <— varian query
    ...
    public function qrStart(WahaSender|string $senderOrSession): array
{
    ...
    // 1) Start session (akan coba POST/GET dgn berbagai payload oleh sessionStart)
    $start = $this->sessionStart($session);

    // Kalau gagal mulai, langsung balikan error
    if (!$start['success']) {
        return $start;
    }

    // Kalau sudah ada QR di response start — langsung kembalikan
    if (!empty($start['qr'])) {
        return $start;
    }

    // 2) Beberapa WAHA tidak mengembalikan QR saat start.
    // Lakukan up-to-3 polling cepat ke /qr untuk ambil QR.
    for ($i = 0; $i < 3; $i++) {
    ...
    // 3) Kalau belum dapat QR, tetap kembalikan hasil start (UI bisa lanjut polling status)
    return $start;
}
