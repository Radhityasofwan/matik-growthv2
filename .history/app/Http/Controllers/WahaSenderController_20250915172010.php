...
class WahaSenderController extends Controller
{
    ...
    public function store(WahaSenderRequest $request)
    {
        ...
        // Jangan menetapkan session tetap; biarkan kosong/placeholder.
        // Jika kolom NOT NULL/UNIQUE, gunakan placeholder sementara — nanti akan DIGANTI di qrStart().
        $placeholder = 'pending-'.Str::uuid();

        if (Schema::hasColumn('waha_senders', 'session')) {
            // Jika nullable -> null; jika tidak -> placeholder aman unik
            try {
                $base['session'] = null;
            } catch (\Throwable $e) {
    ...
    {
        try {
            // === Opsi B: tetapkan session key KANONIK saat akan memulai ===
            // Aturan: sender-{id}-{slug(nama)}; dibersihkan ke [A-Za-z0-9._-], max 64 char
            $canonical = $this->canonicalSessionKey($wahaSender);

            $changed = false;
            if (Schema::hasColumn('waha_senders', 'session')) {
                if ($wahaSender->session !== $canonical) {
                    $wahaSender->session = $canonical;
                    $changed = true;
                }
            }
            if (Schema::hasColumn('waha_senders', 'session_name')) {
                if ($wahaSender->session_name !== $canonical) {
                    $wahaSender->session_name = $canonical;
                    $changed = true;
                }
            }
            if ($changed) {
            ...
            // Response shape sudah: success, connected, state, qr, error
