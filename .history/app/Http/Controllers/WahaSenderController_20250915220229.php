    ...
    public function qrImage(WahaSender $wahaSender)
{
    ...
    if ($img['success'] && !empty($img['body'])) {
        // ⛔ jangan cache, QR berubah-ubah
        return response($img['body'], 200)
            ->header('Content-Type', $img['ctype'] ?: 'image/png')
            ->header('Cache-Control', 'no-store, no-cache, must-revalidate, max-age=0');
    }

    // 404 lebih jelas untuk <img> daripada 204 (yang terlihat "blank" selamanya)
...
public function qrStatus(WahaSender $wahaSender)
{
    try {
        ...
        // siapkan qr_url fallback untuk SCAN_QR_CODE
        if (($resp['success'] ?? false)
            && strtoupper((string)($resp['state'] ?? '')) === 'SCAN_QR_CODE'
            && empty($resp['qr'])) {
            $resp['qr_url'] = route('waha-senders.qr-image', $wahaSender);
        ...
        // ✅ kalau sudah tersambung, tandai aktif dan isi nomor/display_name (sekali saja)
        if (($resp['success'] ?? false) && ($resp['connected'] === true
            || in_array(strtoupper((string)($resp['state'] ?? '')), ['CONNECTED','READY','WORKING','OPEN','AUTHENTICATED','ONLINE','LOGGED_IN','RUNNING'], true))) {
