...
class BroadcastController extends Controller
{
    ...
    {
        $templates = WATemplate::query()->orderBy('name')->get(['id','name','description']);
    ...
    public function store(Request $request)
    {
        $data = $request->validate([
            ...
            'template_id'  => ['required_if:mode,template','nullable','exists:w_a_templates,id'],
            'params_json'  => ['nullable','string'], // JSON object untuk template params
            'recipients'   => ['required','string'], // satu nomor per baris, opsional ada nama
        ], [
        ...
        if ($mode === 'template') {
            ...
            foreach ($recipients as $r) {
                try {
                    ...
                    $responses[] = ['to' => $r['phone'], 'ok' => (bool)$resp, 'resp' => $resp];
                    $resp ? $sent++ : $failed++;
                } catch (\Throwable $e) {
                    Log::error('Broadcast template failed', ['e' => $e->getMessage(), 'to' => $r['phone']]);
                    $responses[] = ['to' => $r['phone'], 'ok' => false, 'resp' => $e->getMessage()];
                    $failed++;
                }
            }
        } else {
            // custom message dengan placeholder {{name}}
