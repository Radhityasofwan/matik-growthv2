...
class BroadcastController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Parse & normalisasi penerima (nama, telp | telp, nama | telp)
        ...
        foreach ($rows as $row) {
            ...
            if ($phone) {
                // normalisasi 08xxxx -> 628xxxx (Indonesia) agar sinkron dengan WAHA
        ...
        if (!empty($data['params_json'])) {
            ...
            if (is_array($decoded)) {
                ...
                // jika invalid, fallback kosong tanpa error
                Log::warning('Invalid params_json, ignored', ['raw'=>$data['params_json']]);
            }
        }
        ...
        if ($data['mode'] === 'template') {
            ...
            foreach ($recipients as $r) {
                // context prioritas: global -> recipient
                ...
                try {
                    ...
                    if (($resp['success'] ?? false) === true) {
                        ...
                        $failed++;
                        Log::warning('WAHA send failed (template)', [
                            'to'   => $r['phone'],
                            'err'  => $resp['error'] ?? null,
                            'raw'  => $resp['raw'] ?? null,
                            'sess' => $sender->session ?? $sender->session_name ?? null,
                        ]);
                    }
                    // jeda ringan agar tidak dianggap spam oleh sebagian server
                    usleep(250 * 1000);
                } catch (\Throwable $e) {
            ...
            foreach ($recipients as $r) {
                // dukung {{name}}, @{{name}}, {{nama}}, {{nama_pelanggan}}
                $text = strtr($message, [
                    '{{name}}'           => $r['name'],
                    '@{{name}}'          => $r['name'],
                    '{{nama}}'           => $r['name'],
                ...
                try {
                    ...
                    if (($resp['success'] ?? false) === true) {
                        ...
                        $failed++;
                        Log::warning('WAHA send failed (custom)', [
                            'to'   => $r['phone'],
                            'err'  => $resp['error'] ?? null,
                            'raw'  => $resp['raw'] ?? null,
                            'sess' => $sender->session ?? $sender->session_name ?? null,
        ...
        $flashKey = $failed > 0 ? 'error' : 'success'; // gagal → merah
        return redirect()->route('whatsapp.broadcast.create')->with($flashKey, $msg);
    }

    /**
     * Render {{variables}} dengan fallback kosong jika variabel tidak ada.
     * Jika $allowedVars diberikan dari WATemplate, batasi hanya yang diizinkan.
     */
    protected function renderTemplate(string $body, array $context, array $allowedVars = []): string
    {
        ...
        // Kalau ada daftar variables dari template, gunakan itu; kalau tidak, ekstrak semua.
        if (!$allowedVars) {
        ...
        foreach ($allowedVars as $k) {
            ...
            $repls['{{'.$k.'}}']  = (string) $val;
            $repls['@{{'.$k.'}}'] = (string) $val; // dukung prefiks @
        }
