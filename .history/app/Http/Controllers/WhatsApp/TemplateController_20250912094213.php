...
class TemplateController extends Controller
{
    ...
    public function store(WATemplateRequest $request)
    {
        ...
        // Pastikan is_active ada dan bertipe bool (default true jika tidak disertakan form)
        $data['is_active'] = array_key_exists('is_active', $data)
            ? (bool) $data['is_active']
            : true;

        // Ekstraksi variabel dari body
    ...
    public function update(WATemplateRequest $request, WATemplate $template)
    {
        ...
        // Pastikan is_active ada dan bertipe bool (tetap nilai lama jika tidak dikirim form)
        $data['is_active'] = array_key_exists('is_active', $data)
            ? (bool) $data['is_active']
            : $template->is_active;
    ...
    {
        // Regex lebih toleran: dukung spasi di dalam {{ }}, titik, dash, underscore.
        // Contoh yang tertangkap: {{name}}, {{ name }}, {{first-name}}, {{user.email}}
        preg_match_all('/\{\{\s*([A-Za-z0-9_.-]+)\s*\}\}/', $body, $matches);

        // Kembalikan nama variabel unik (tanpa kurung kurawal)
