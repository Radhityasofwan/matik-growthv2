...
class BroadcastController extends Controller
{
    ...
    public function create()
    {
        ...
        // ----- Senders (aktif, urut default -> name/id; alias session_name -> session) -----
        ...
        if (Schema::hasColumn('waha_senders', 'name')) $sq->orderBy('name');
        else                                           $sq->orderBy('id');

        $senders = $sq->get($senderCols);

        // View name fallback (opsional)
        $viewName = 'whatsapp.broadcast.create';
        if (!View::exists($viewName)) {
            $alt = 'whatsapp.broadcast';
            $viewName = View::exists($alt) ? $alt : (View::exists('broadcast') ? 'broadcast' : $viewName);
        }
