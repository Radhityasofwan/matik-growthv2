...
class WahaSenderController extends Controller
{
    ...
    public function index(Request $request)
    {
        if ($request->wantsJson() || $request->boolean('json')) {
            ...
            if (Schema::hasColumn('waha_senders', 'session')) {
                $cols[] = 'session';
            } elseif (Schema::hasColumn('waha_senders', 'session_name')) {
    ...
    private function normalize(array $data, ?int $ignoreId = null): array
    {
        ...
        // map session -> session_name jika kolom session tidak ada
        if (isset($data['session'])) {
            if (!Schema::hasColumn('waha_senders', 'session') && Schema::hasColumn('waha_senders', 'session_name')) {
