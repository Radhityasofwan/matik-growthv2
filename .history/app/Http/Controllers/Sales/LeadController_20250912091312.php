...
class LeadController extends Controller
{
    ...
    public function store(LeadRequest $request)
    {
        ...
        // Simpan tanpa registered_at (bukan kolom DB)
        $registeredAt = $request->input('registered_at');
        unset($data['registered_at']);

        // Pastikan status manual tidak mengunci status 'trial' jika dipakai internal
        // (boleh saja: jika tidak diisi manual, biarkan default 'trial' via UI/logic Anda)
        $lead = Lead::create($data);

        // Set created_at jika diberikan
        if ($registeredAt) {
            $lead->created_at = Carbon::parse($registeredAt);
            // Pastikan trial_ends_at minimal = created_at
    ...
    public function update(LeadRequest $request, Lead $lead)
    {
        ...
        // Validasi logis: trial_ends_at >= created_at (jika registeredAt diberikan)
        ...
        // Jika status converted, buat/ubah subscription (tetap seperti model sebelumnya)
    ...
    public function import(Request $request)
    {
        ...
        $parseDate = function ($v, bool $withTime=false) {
            ...
            foreach ($formats as $f) { try { return Carbon::createFromFormat($f, trim((string)$v)); } catch (\Throwable $e) {} }
