...
{
    // Tabel activity Spatie (fallback ke default jika config tak ada)
    ...
    $query = Lead::query()
        ...
        // === hitung jumlah chat WA berbasis activity log
        ->selectSub(function ($q) use ($activityTable) {
            $q->from($activityTable)
              ->selectRaw('COUNT(*)')
              ->whereColumn("{$activityTable}.subject_id", 'leads.id')
              ->where("{$activityTable}.subject_type", Lead::class)
              ->where("{$activityTable}.log_name", 'wa_chat');
        }, 'chat_count')

        // === cap waktu chat terakhir
        ->selectSub(function ($q) use ($activityTable) {
            $q->from($activityTable)
        ...
        // === cap waktu balasan terakhir (sesuaikan log_name jika inbound kamu beda)
        ->selectSub(function ($q) use ($activityTable) {
            $q->from($activityTable)
              ->selectRaw('MAX(created_at)')
              ->whereColumn("{$activityTable}.subject_id", 'leads.id')
    ...
    // Smart filter: chat_status (pakai HAVING untuk alias dari selectSub)
    ...
    // Stabil: jika kolom 'name' tidak ada pada instalasi lama, fallback ke 'id'
