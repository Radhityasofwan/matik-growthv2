...
use App\Models\WahaSender; // penting: model yang benar
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Schema; // ✅ untuk cek kolom secara aman
...
class LeadController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // ✅ Stabil: jika kolom `name` tidak ada di DB (kasus Anda), fallback ke `id`
    ...
    public function store(LeadRequest $request)
    {
        ...
        // Redirect tanpa fragment supaya modal tidak terbuka lagi
    ...
    public function destroy(Lead $lead)
    {
        DB::transaction(function () use ($lead) {
            // Hapus relasi yang mungkin menghalangi FK
    ...
    public function import(Request $request)
    {
        ...
        $parseDate = function ($v, bool $withTime = false) {
            ...
            foreach ($formats as $f) {
                try {
                    return Carbon::createFromFormat($f, trim((string) $v));
                } catch (\Throwable $e) {
                }
            }
            try {
        ...
        try {
            foreach (array_slice($rows, 1) as $row) {
                ...
                // Fallback name wajib: store_name -> ownerName -> before @
                ...
                if ($lead) {
                    // Update: jaga name existing, tapi isi jika kosong
