...
{
    /**
     * Gunakan timezone dari config (contoh: 'Asia/Jakarta').
     */
    ...
    protected function schedule(Schedule $schedule): void
    {
        ...
        // Deteksi task overdue berkala (quarter-hour)
        ...
        // Konsolidasi logika trial
        $schedule->command('leads:update-trial-statuses')
            ->dailyAt('08:00')
            ->onOneServer()->withoutOverlapping();

        // Kirim reminder campaign harian
        $schedule->command('campaigns:send-reminders')
            ->dailyAt('09:05')
            ->onOneServer()->withoutOverlapping();

        // Sweep WA Task: H-1 due & H+1 overdue (dedup harian)
        // → Pastikan command ini yang melakukan query tanggal lalu dispatch
        //   SendTaskWhatsAppJob untuk event: due_h1 / overdue_h1 sesuai kriteria.
        $schedule->command('tasks:sweep-due')
            ->dailyAt('09:00')
            ->onOneServer()->withoutOverlapping();

        // Cek overdue via command per jam (pelengkap)
