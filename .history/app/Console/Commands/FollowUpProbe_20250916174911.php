...
class FollowUpProbe extends Command
{
    protected $signature = 'followup:probe
        ...
        {--owner-wa=628111111111 : Nomor WA owner untuk tes}
    ...
    public function handle(): int
    {
        ...
        $ownerWa  = preg_replace('/\D+/', '', (string)$this->option('owner-wa')) ?: '628111111111';
        ...
        /* ===== 1) Siapkan OWNER + SENDER + TEMPLATE (minimal) ===== */
        /** @var User $owner */
        ...
        /** @var WahaSender $sender */
        $sender = WahaSender::firstOrCreate(
            ['name' => 'Sender Probe'],
            ['number' => '628222222222', 'is_active' => true, 'is_default' => true, 'session' => 'sender-probe']
        );

        // Template: pakai WATemplate jika ada, fallback Template bila berbeda di project
        ...
        if ($leadId) {
            ...
            if (!$lead->phone) { $lead->phone = '628333333333'; }
        ...
        $ownerLog = $logs->firstWhere('log_name', 'follow_up_notify_owner');    // ke OWNER (jalur lama)
