...
class SendLeadFollowUpJob implements ShouldQueue
{
    ...
    protected function processRule(LeadFollowUpRule $rule): void
    {
        ...
        foreach ($eligible as $lead) {
            try {
                ...
                // Notify owner (jika punya wa_number)
    ...
    /**
     * Ambil leads yang memenuhi kondisi (pakai alias 'meta_*' agar tidak bentrok
     * dengan kolom asli seperti 'chat_count' di tabel leads).
     */
    protected function eligibleLeadsFor(LeadFollowUpRule $rule)
    {
        ...
        // Subselect: beri alias "meta_*" untuk hindari duplicate column name
        $sub = Lead::query()
            ->select('leads.*')
