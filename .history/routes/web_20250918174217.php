...
Route::middleware('auth')->group(function () {
    ...
    // edit (JSON) tetap tersedia dengan nama khusus
    Route::get('tasks/{task}/edit', [TaskController::class, 'edit'])->name('tasks.edit');

    // Update status via AJAX drag-n-drop
    // Sediakan PATCH (semantik REST) dan POST (kompatibilitas) ke method yang sama.
    Route::patch('tasks/{task}/status', [TaskController::class, 'updateStatus'])->name('tasks.status.update');
    Route::post ('tasks/{task}/update-status', [TaskController::class, 'updateStatus'])->name('tasks.updateStatus'); // backward compat
    ...
    // Endpoint internal untuk job/kode yang membuat Request::create('/_internal/task-notify', ...)
    // Supaya bisa juga dipanggil via HTTP jika diperlukan (tetap berada di balik auth).
    Route::post('_internal/task-notify', [WahaController::class, 'sendMessage'])->name('internal.task-notify');
    // (opsional) alias GET untuk health-check WA
