...
Route::middleware('auth')->group(function () {
    ...
    /* ===================== Tasks ===================== */
    Route::middleware(['auth']) // jika memang pakai auth
    ->group(function () {

    // Resource standar (index, store, edit(json via Accept: application/json), update, destroy)
    Route::resource('tasks', TaskController::class)
        ->except(['show', 'create']);

    // Update status (drag & drop / menu)
    // Terima PATCH & POST ke method yang sama, satu nama utama:
    Route::match(['patch', 'post'], 'tasks/{task}/status', [TaskController::class, 'updateStatus'])
        ->name('tasks.status.update');

    // (Opsional) alias kompatibilitas lama — kalau frontend lama masih memanggil ini
    Route::post('tasks/{task}/update-status', [TaskController::class, 'updateStatus'])
        ->name('tasks.updateStatus'); // boleh dihapus kalau sudah tidak dipakai
