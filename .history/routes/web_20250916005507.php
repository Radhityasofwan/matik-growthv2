...
Route::middleware('auth')->group(function () {
    ...
    */
    ...
    Route::get('/waha-senders/{wahaSender}/qr-image', [\App\Http\Controllers\WahaSenderController::class, 'qrImage'])
    ->name('waha-senders.qr-image');

    Route::post('/waha-senders/{wahaSender}/auth-request-code',
    [\App\Http\Controllers\WahaSenderController::class, 'authRequestCode'])
    ->name('waha-senders.auth-request-code');

    Route::get('/debug/waha-send', function (Request $r) {
    abort_unless(app()->environment(['local','testing']), 403); // jangan dibiarkan di production

    $sender = WahaSender::query()
        ->when($r->id, fn($q)=>$q->where('id',$r->id))
        ->when(!$r->id, fn($q)=>$q->where('is_active',true))
        ->firstOrFail();

    $to   = $r->input('to','62851234xxxx');
    $text = $r->input('text','Halo! WAHA aktif 🎉');

    $svc  = app(\App\Services\WahaService::class);
    $res  = $svc->sendMessage($sender, $to, $text);

    return response()->json(['sender_id'=>$sender->id, 'to'=>$to, 'result'=>$res]);

    // (Opsional) Logout/Disconnect sesi via modal — aktifkan bila controllernya sudah ada
    // Route::post('waha-senders/{wahaSender}/qr-logout', [WahaSenderController::class, 'qrLogout'])
    //     ->name('waha-senders.qr-logout');
});
