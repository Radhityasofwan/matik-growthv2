...
Route::middleware('auth')->group(function () {
    ...
    // WhatsApp Templates / Broadcast / Logs
    ...
    // Manual Broadcast
    Route::get('whatsapp/broadcast/manual', [BroadcastController::class, 'createManual'])->name('whatsapp.broadcast.create_manual');
    Route::post('whatsapp/broadcast/manual', [BroadcastController::class, 'storeManual'])->name('whatsapp.broadcast.store_manual');
    ...
    // Manajemen Waha Sender (CRUD + endpoint JSON untuk dropdown di Leads)
    Route::resource('waha-senders', WahaSenderController::class)->except(['show','create','edit']);
    ...
    Route::middleware('auth')->group(function () {
    Route::prefix('waha-senders')->name('waha-senders.')->group(function () {
        Route::get('/', [WahaSenderController::class, 'index'])->name('index');                        // JSON/HTML
        Route::post('/', [WahaSenderController::class, 'store'])->name('store');                       // create
        Route::post('{wahaSender}', [WahaSenderController::class, 'update'])->name('update');          // update (PUT spoof)
        Route::delete('{wahaSender}', [WahaSenderController::class, 'destroy'])->name('destroy');      // delete
        Route::post('{wahaSender}/toggle-active', [WahaSenderController::class, 'toggleActive'])->name('toggle-active');
        Route::post('{wahaSender}/set-default',  [WahaSenderController::class, 'setDefault'])->name('set-default');
    });
});
