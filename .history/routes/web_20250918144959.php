...
Route::middleware('auth')->group(function () {
    ...
    // Content & Tasks
    Route::resource('tasks', \App\Http\Controllers\Content\TaskController::class)->except(['show', 'create']);
    Route::post('tasks/{task}/update-status', [\App\Http\Controllers\Content\TaskController::class, 'updateStatus'])->name('tasks.update-status');

    // Task Dropdown Routes
    Route::post('/tasks/{task}/toggle-pin', [\App\Http\Controllers\Content\TaskController::class, 'togglePin'])->name('tasks.togglePin');
    Route::post('/tasks/{task}/duplicate', [\App\Http\Controllers\Content\TaskController::class, 'duplicate'])->name('tasks.duplicate');

    // NEW: Update icon (emoji)
    Route::post('/tasks/{task}/update-icon', [\App\Http\Controllers\Content\TaskController::class, 'updateIcon'])->name('tasks.updateIcon');

    // NEW: Comment (store)
    Route::post('/tasks/{task}/comments', [\App\Http\Controllers\Content\TaskController::class, 'addComment'])->name('tasks.comments.store');
