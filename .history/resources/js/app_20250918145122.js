...
const TaskBoard = (() => {
  ...
  function promptAndUpdateStatus(taskId, status, element) {
    ...
    if (['in_progress','review','done'].includes(status)) {
      if (confirm('Kirim notifikasi WA ke PIC dan Owner untuk perubahan status ini?')) {
        notify_wa = 1;
      }
    }
  ...
  async function openEditModal(taskId) {
    ...
    // tanggal dari JSON (ISO 8601) -> YYYY-MM-DD
    const toDateInput = (v) => {
      if (!v) return '';
      // ex "2025-09-18T14:25:00.000000Z" -> "2025-09-18"
      return String(v).split('T')[0] || String(v).split(' ')[0] || '';
    };

    form.querySelector('#edit_title').value = task.title || '';
    form.querySelector('#edit_progress').value = task.progress || 0;
    form.querySelector('#edit_start_at').value = toDateInput(task.start_at);
    form.querySelector('#edit_due_date').value = toDateInput(task.due_date);
  ...
  // --- Icon (emoji) ---
  async function editIcon(taskId) {
    const icon = prompt('Masukkan emoji/icon untuk task ini (kosongkan untuk menghapus):', '');
    if (icon === null) return;
    const data = await callTaskApi(`/tasks/${taskId}/update-icon`, 'POST', { icon });
    if (data?.success) {
      showToast('Icon tugas diperbarui.');
  ...
  async function submitComment() {
    const modal = document.getElementById('comment_modal');
    const textarea = modal?.querySelector('#comment_content');
    const content = (textarea?.value || '').trim();
    if (!commentTaskId || !content) {
      alert('Komentar tidak boleh kosong.');
      return;
    }
    const data = await callTaskApi(`/tasks/${commentTaskId}/comments`, 'POST', { content });
    if (data?.success) {
      modal.close();
...
window.openCommentModal = TaskBoard.openCommentModal;
window.submitComment = TaskBoard.submitComment;
