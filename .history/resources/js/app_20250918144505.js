...
Alpine.data('notifBell', (el) => {
  ...
  const feedUrlAttr = el?.dataset?.feed || ''
  const readBaseAttr = el?.dataset?.readBase || ''
  const markAllUrlAttr = el?.dataset?.readAll || ''
  ...
  return {
    ...
    feedUrl() { return feedUrlAttr },
    readUrl(id) { return readBaseAttr ? `${readBaseAttr}/${encodeURIComponent(id)}/read` : '' },
    markAllUrl() { return markAllUrlAttr },
...
const TaskBoard = (() => {
  ...
  async function callTaskApi(url, method = 'POST', body = {}) {
    ...
    // Kirim body (JSON) untuk semua method kecuali GET tanpa body
    if (method !== 'GET') {
    ...
    try {
      ...
      // bisa 204 (hapus) → kembalikan null
  ...
  function promptAndUpdateStatus(taskId, status, element) {
    // Kembalikan dialog konfirmasi untuk WA — seperti sebelumnya
    let notify_wa = 0
  ...
  async function openEditModal(taskId) {
    ...
    const form = modal.querySelector('form#edit_task_form')
    form.action = `/tasks/${taskId}`

    // Isi field
    const $ = (sel) => form.querySelector(sel)
    $('#edit_title').value = task.title || ''
    $('#edit_progress').value = task.progress ?? 0
    $('#edit_start_at').value = task.start_at ? String(task.start_at).split(' ')[0] : ''
    $('#edit_due_date').value = task.due_date ? String(task.due_date).split(' ')[0] : ''
    $('#edit_priority').value = task.priority || 'medium'
    $('#edit_assignee_id').value = task.assignee_id || ''
    $('#edit_icon').value = task.icon || ''   // pastikan field ada di modal
    $('#edit_color').value = task.color || '' // opsional, jika tersedia
    $('#edit_link').value = task.link || ''   // opsional, jika tersedia
  ...
  async function togglePin(taskId) {
    ...
    if (data?.success) {
      ...
      // reload agar urutan (order:-1) tersusun ulang
      location.reload()
    }
  }
  ...
  function copyLink(taskId) {
    ...
    navigator.clipboard.writeText(url)
      .then(() => showToast('Link tugas berhasil disalin!'))
      .catch(err => {
  ...
  function commentOnTask(taskId) {
    // Placeholder — tersambung ke UI
    // Integrasi komentar backend bisa ditambahkan terpisah
    showToast('Fitur komentar akan segera hadir.')
  }

  function editIcon(taskId) {
    // Buka modal edit & fokuskan field icon agar UX sesuai menu "Edit icon"
    openEditModal(taskId).then(() => {
      const iconField = document.querySelector('#edit_task_modal #edit_icon')
      if (iconField) iconField.focus()
    })
  }
...
window.editIcon = TaskBoard.editIcon
