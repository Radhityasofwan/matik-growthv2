...
@section('content')
@php
    // Data preparation logic from your original file remains unchanged
    $stats = $stats ?? [];
...
<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
    <div data-aos="fade-up" data-aos-delay="50">
        <x-card.stat
            title="Total Leads"
            value="{{ number_format($stats['totalLeads'] ?? 0) }}"
            change="{{ $leadsChange >= 0 ? '▲' : '▼' }} {{ number_format(abs($leadsChange)) }} minggu ini"
            changeType="{{ $leadsChange >= 0 ? 'success' : 'error' }}"
        >
    ...
    <div data-aos="fade-up" data-aos-delay="100">
        <x-card.stat
            title="Pesan Terkirim (7h)"
            value="{{ number_format($stats['messagesSentLast7Days'] ?? 0) }}"
            change="{{ $sentChange >= 0 ? '▲' : '▼' }} {{ number_format(abs($sentPct), 1) }}% vs 7 hari lalu"
            changeType="{{ $sentChange >= 0 ? 'success' : 'error' }}"
        >
    ...
    <div data-aos="fade-up" data-aos-delay="150">
        <x-card.stat
            title="Tingkat Balasan"
            value="{{ number_format($stats['replyRate'] ?? 0, 1) }}%"
            change="{{ $replyChange >= 0 ? '▲' : '▼' }} {{ number_format(abs($replyChange), 1) }}% vs 7 hari lalu"
            changeType="{{ $replyChange >= 0 ? 'success' : 'error' }}"
        >
    ...
    <div data-aos="fade-up" data-aos-delay="200">
        <x-card.stat
            title="{{ is_null($connected) ? 'WA Sender Aktif' : 'WA Sender Tersambung' }}"
            value="{{ $senderLabelCount }}"
            change="{{ $senderLabelDesc }}"
        >
     ...
     <div data-aos="fade-up" data-aos-delay="250">
        <x-card.stat
            title="MRR (Active)"
            value="Rp {{ number_format($subSummary['mrr'] ?? 0, 0, ',', '.') }}"
            change="Subs aktif: {{ number_format($subSummary['active'] ?? 0) }}"
        >
             <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5 text-primary" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="1" x2="12" y2="23"></line><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"></path></svg>
        </x-card.stat>
    </div>
     <div data-aos="fade-up" data-aos-delay="300">
        <x-card.stat
            title="Tugas Terbuka"
            value="{{ ($taskSummary['open'] ?? 0) + ($taskSummary['in_progress'] ?? 0) }}"
            change="Open: {{ $taskSummary['open'] ?? 0 }} • Progress: {{ $taskSummary['in_progress'] ?? 0 }}"
        >
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
    ...
    <div class="card bg-base-100 shadow-sm border border-base-300/50 lg:col-span-2" data-aos="fade-up">
        <div class="card-body">
            ...
            <div x-data="mainChart(@json($chartData))" x-init="init()" class="w-full h-80 min-h-[320px]">
    ...
    <div class="card bg-base-100 shadow-sm border border-base-300/50" data-aos="fade-up" data-aos-delay="100">
        <div class="card-body">
            ...
            <div x-data="statusDonut(@json($statusDonut))" x-init="init()" class="w-full h-80 min-h-[320px]">
    ...
    <div class="card bg-base-100 shadow-sm border border-base-300/50" data-aos="fade-up">
        <div class="card-body">
            ...
            @if($trialsSoon->isEmpty())
                <div class="text-center py-8 text-neutral/60">Tidak ada trial yang akan berakhir.</div>
            @else
                    ...
                    <table class="table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-right">Berakhir Dalam</th>
                            </tr>
                        </thead>
                        <tbody>
                        @foreach($trialsSoon as $lead)
                            <tr>
                                ...
                                <td class="hidden sm:table-cell text-neutral/80">{{ $lead->store_name }}</td>
                                <td class="text-right text-neutral/80">{{ optional($lead->trial_ends_at)->diffForHumans(['short' => true]) }}</td>
                            </tr>
                        @endforeach
                        </tbody>
                    </table>
    ...
    <div class="card bg-base-100 shadow-sm border border-base-300/50" data-aos="fade-up" data-aos-delay="100">
        <div class="card-body">
            ...
            @if(empty($taskSummary['mineToday']) || collect($taskSummary['mineToday'])->isEmpty())
                <div class="text-center py-8 text-neutral/60">Tidak ada tugas jatuh tempo hari ini.</div>
            @else
                    ...
                    @foreach($taskSummary['mineToday'] as $t)
                        <li class="flex items-center justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-neutral/60">{{ strtoupper($t->status) }}</p>
                            </div>
...
<!-- Aktivitas Terbaru -->
<div class="card bg-base-100 shadow-sm border border-base-300/50 mt-6" data-aos="fade-up">
    <div class="card-body">
        ...
        <div class="space-y-4">
            @forelse($recentActivities as $activity)
                <div class="flex items-start gap-4">
                    <div class="avatar placeholder">
                        <div class="bg-secondary text-primary rounded-full w-10">
                    ...
                    <div>
                        <p class="text-sm text-neutral break-words">
                            @if($activity->causer)
            ...
            @empty
                <div class="text-center text-neutral/60 py-8">Belum ada aktivitas terbaru.</div>
            @endforelse
        </div>
    </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        // AlpineJS component for Main Area Chart
        Alpine.data('mainChart', (chartData) => ({
            isLoading: true,
            init() {
                // Simulate loading then render chart
                setTimeout(() => {
        ...
        // AlpineJS component for Status Donut Chart
