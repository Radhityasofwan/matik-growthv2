...
@php
    ...
    $connected = $stats['connectedSenders'];
    $active    = $stats['activeSenders'] ?? 0;
    $total     = $stats['totalSenders'] ?? 0;

    // Teks status sender
    $senderLabelCount = is_null($connected)
        ? "{$active} / {$total}" // fallback jika tidak diprobe WAHA
        : "{$connected} / {$total}";
...
@endphp
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <!-- Kartu 4: WA Sender (Connected / Total atau Active / Total) -->
        <div class="card bg-base-100 shadow-md border border-base-300/50">
            <div class="card-body">
                <div class="flex items-center justify-between">
                    <h2 class="card-title text-sm font-semibold text-neutral/60">
                        {{ is_null($connected) ? 'WA Sender Aktif' : 'WA Sender Tersambung' }}
                    </h2>
                    <div class="bg-secondary p-2 rounded-lg">
                ...
                <p class="text-4xl font-bold text-neutral mt-2">{{ $senderLabelCount }}</p>
