...
@php
    ...
    // Pastikan bentuknya Collection supaya foreach/isEmpty aman
    $taskSummary            = $taskSummary ?? ['open'=>0,'in_progress'=>0,'done'=>0,'mineToday'=>collect()];
    $tasksToday             = collect($taskSummary['mineToday'] ?? []);
    ...
    $sentPct     = ($stats['messagesSentPrevious7Days'] ?? 0) > 0
        ? ($sentChange / max(1, ($stats['messagesSentPrevious7Days'] ?? 1))) * 100
        : 0;
    $replyChange = ($stats['replyRate'] ?? 0) - ($stats['replyRatePrevious'] ?? 0);

    // ===== Label status WA Sender =====
                        ...
                        @foreach($trialsSoon as $lead)
                            <tr>
                                ...
                                <td class="text-right text-base-content/80">
                                    {{ optional($lead->trial_ends_at)->diffForHumans(['short' => true]) }}
                                </td>
                            </tr>
                        @endforeach
                        </tbody>
                ...
                <div class="flex items-start gap-4">
                    <div class="avatar placeholder">
                        <div class="bg-secondary text-secondary-content rounded-full w-10 h-10 flex items-center justify-center">
                            @php
                                $d = strtolower($activity->description ?? '');
                                $isSend = (strpos($d, 'broadcast') !== false) || (strpos($d, 'kirim') !== false) || (strpos($d, 'wa') !== false);
                                $isImport = (strpos($d, 'import') !== false);
                            @endphp
                            @if($isSend)
...
@push('scripts')
{{-- Pastikan ApexCharts ready sebelum Alpine render --}}
...
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('mainChart', () => ({
        ...
        init() {
            let chartData;
            try { chartData = JSON.parse(this.$el.dataset.chartData || '{}'); }
            catch (e) { console.error('Error parsing chart data JSON:', e); this.isLoading = false; return; }
    ...
    Alpine.data('statusDonut', () => ({
        ...
        init() {
            let donutData;
            try { donutData = JSON.parse(this.$el.dataset.donutData || '{}'); }
            catch (e) { console.error('Error parsing donut data JSON:', e); this.isLoading = false; return; }
