...
function getCssColor(cls, prop='color', fallback='#999') {
    ...
    el.remove();
    return val || fallback; // e.g. "rgb(59, 130, 246)"
}
function rgbaFrom(rgbStr, alpha=1) {
    const m = rgbStr?.match(/rgba?\((\d+),\s*(\d+),\s*(\d+)/);
    if (!m) return rgbStr || `rgba(0,0,0,${alpha})`;
    const [, r, g, b] = m.map(Number);
    return `rgba(${r}, ${g}, ${b}, ${alpha})`;
}
function snapshotTheme() {
    const primary   = getCssColor('text-primary',   'color',           '#3B82F6');
    const secondary = getCssColor('text-secondary', 'color',           '#06B6D4');
    const accent    = getCssColor('text-accent',    'color',           '#F472B6');
    const info      = getCssColor('text-info',      'color',           '#0EA5E9');
    const success   = getCssColor('text-success',   'color',           '#22C55E');
    const warning   = getCssColor('text-warning',   'color',           '#F59E0B');
    const error     = getCssColor('text-error',     'color',           '#EF4444');
    const base      = getCssColor('text-base-content','color',         '#374151');
    const bg        = getCssColor('bg-base-100',    'backgroundColor', '#ffffff');

    // Heuristik mode gelap berdasarkan luma background
    const m = bg.match(/\d+/g), dark = m ? (0.2126*m[0]/255 + 0.7152*m[1]/255 + 0.0722*m[2]/255) < 0.5 : false;

    return {
        isDark: dark,
        label: rgbaFrom(base, 0.75),
        grid : rgbaFrom(base, 0.12),
        base,
        colors: { primary, secondary, accent, info, success, warning, error }
    };
}

/* ===================== CHARTS (Alpine Components) ===================== */
document.addEventListener('alpine:init', () => {
    // ---- Area Chart: Aktivitas 30 Hari ----
    Alpine.data('apexChart', () => ({
        ...
        init() {
            ...
            const boot = () => {
                ...
                this.theme = snapshotTheme();
                this.$nextTick(() => this.render(data));
            };
            if (window.ApexCharts) boot(); else document.addEventListener('DOMContentLoaded', boot);

            // Redraw saat tema berubah
            new MutationObserver(() => this.render(data))
        ...
        render(data) {
            ...
            const options = {
                ...
                xaxis: {
                    ...
                    labels: { style: { colors: t.label } },
                    axisBorder: { show: false },
                    axisTicks: { show: false },
                    crosshairs: { stroke: { color: t.grid, width: 1, dashArray: 4 } },
                    tooltip: { enabled: false }
                },
                yaxis: { labels: { style: { colors: t.label } }, min: 0, forceNiceScale: true },
                tooltip: {
                ...
                legend: { labels: { colors: t.label }, position: 'top', horizontalAlign: 'right' },
                responsive: [
    ...
    Alpine.data('donutChart', () => ({
        ...
        render(data) {
            ...
            if (!data.series?.length || data.series.every(v => Number(v) === 0)) {
            ...
            const donutColors = [
                ...
                t.colors.info, t.colors.success, t.colors.warning, t.colors.error
            ];
            ...
            const options = {
                ...
                colors: donutColors,
                legend: { position: 'bottom', labels: { colors: t.label }, markers: { radius: 10 } },
                dataLabels: {
                    enabled: true,
                ...
                plotOptions: {
                    pie: {
                        ...
                        donut: {
                            ...
                            labels: {
                                show: true,
                                name: { show: true, color: t.label, offsetY: 8 },
                                value: {
                                    show: true,
                ...
                tooltip: { theme: t.isDark ? 'dark' : 'light', y: { formatter: (v) => `${Number(v||0).toLocaleString()} leads` } },
                fill: { type: 'gradient', gradient: { shade: 'light', type: 'vertical', opacityFrom: 0.95, opacityTo: 0.85, stops: [0,50,100] } },
                responsive: [{ breakpoint: 640, options: { plotOptions: { pie: { donut: { size: '68%' } } } } }]
            };
