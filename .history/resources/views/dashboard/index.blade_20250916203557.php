...
<div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-4 gap-6 mt-6">
                    ...
                    @foreach($taskSummary['mineToday'] as $t)
                        <li class="flex items-center justify-between gap-3">
                            <div class="min-w-0">
                                ...
                                <p class="text-xs text-neutral/60">{{ strtoupper($t->status) }} • Jatuh tempo: {{ optional($t->due_date)->format('d M Y') }}</p>
                            </div>
    ...
    <div class="card bg-base-100 shadow-md border border-base-300/50">
        <div class="card-body">
            ...
            @if(empty($campaignSummary['latest']) || collect($campaignSummary['latest'])->isEmpty())
                <p class="text-neutral/60">Belum ada kampanye.</p>
            @else
                <ul class="space-y-3">
                    @foreach(collect($campaignSummary['latest']) as $c)
        ...
        return {
            init() {
                ...
                if (!chartData || !chartData.categories?.length) {
                    el.innerHTML = '<div class="flex items-center justify-center h-72 text-neutral/60">Data chart tidak tersedia.</div>';
                    return;
                }
                const options = {
                    series: [
        ...
        return {
            init() {
                ...
                if (!d || !d.series?.length) {
                    el.innerHTML = '<div class="flex items-center justify-center h-72 text-neutral/60">Tidak ada data status.</div>';
                    return;
                }
                const options = {
                    series: d.series,
                    ...
                    colors: ['#60A5FA', '#34D399', '#94A3B8', '#F59E0B', '#EF4444'],
