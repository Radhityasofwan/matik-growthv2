...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
    ...
    <div class="card bg-base-100 shadow-lg border border-base-300/50 lg:col-span-2" data-aos="fade-up">
        <div class="card-body">
            ...
            {{-- FIXED: Pass data via data-* attribute --}}
            <div x-data="mainChart" data-chart-data='{{ json_encode($chartData) }}' x-init="init()" class="w-full h-80 min-h-[320px]">
    ...
    <div class="card bg-base-100 shadow-lg border border-base-300/50" data-aos="fade-up" data-aos-delay="100">
        <div class="card-body">
            ...
            {{-- FIXED: Pass data via data-* attribute --}}
            <div x-data="statusDonut" data-donut-data='{{ json_encode($statusDonut) }}' x-init="init()" class="w-full h-80 min-h-[320px] flex items-center justify-center">
...
@push('scripts')
{{-- FIXED: Menghapus CDN karena ApexCharts sudah di-bundle via app.js --}}
<script>
document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('mainChart', () => ({
        ...
        chart: null,
        init() {
            // FIXED: Ambil data dari data-* attribute
            const chartData = JSON.parse(this.$el.dataset.chartData);

            // Cek jika ApexCharts sudah ter-load sebelum melanjutkan
            if (typeof ApexCharts === 'undefined') {
                console.error('ApexCharts library is not loaded from app.js.');
            ...
            // Gunakan MutationObserver untuk mendeteksi perubahan tema secara otomatis
    ...
    Alpine.data('statusDonut', () => ({
        ...
        init() {
            // FIXED: Ambil data dari data-* attribute
            const donutData = JSON.parse(this.$el.dataset.donutData);
