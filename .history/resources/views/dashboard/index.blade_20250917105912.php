...
document.addEventListener('alpine:init', () => {
    // Komponen Alpine untuk Chart Area
    Alpine.data('apexChart', () => ({
        ...
        renderChart(data) {
            ...
            const primary = `hsl(${styles.getPropertyValue('--p').trim()})`;
            const accent = `hsl(${styles.getPropertyValue('--a').trim()})`;
            const labelColor = `hsl(${styles.getPropertyValue('--bc')} / 0.6)`;
            const gridColor = `hsl(${styles.getPropertyValue('--bc')} / 0.1)`;
            ...
            const options = {
                ...
                // KREATIF: Menambahkan marker yang muncul saat di-hover
                ...
                xaxis: {
                    ...
                    // INTERAKTIF: Menambahkan garis vertikal (crosshair) saat hover
                    crosshairs: {
                        show: true,
                        width: 1,
                ...
                // INTERAKTIF: Tooltip yang menampilkan kedua data sekaligus
                tooltip: {
                ...
                legend: { labels: { colors: labelColor }, position: 'top', horizontalAlign: 'right' }
            };
    ...
    // Komponen Alpine untuk Donut Chart
    Alpine.data('donutChart', () => ({
        ...
        renderChart(data) {
            ...
            if (!el || typeof ApexCharts === 'undefined') return;
            if (this.chart) this.chart.destroy();
            if (!data.series?.length || data.series.every(item => item === 0)) {
                el.innerHTML = `<div class="flex items-center justify-center h-full text-base-content/60">Tidak ada data status.</div>`;
                return;
            }
            const styles = getComputedStyle(document.documentElement);
            const labelColor = `hsl(${styles.getPropertyValue('--bc')} / 0.6)`;
            const totalColor = `hsl(${styles.getPropertyValue('--bc')})`;
            const isDark = document.documentElement.classList.contains('dark');
            const donutColors = ['#3B82F6', '#10B981', '#64748B', '#F59E0B', '#EF4444', '#8B5CF6'];
            ...
            const options = {
                ...
                // KREATIF & INTERAKTIF: Menampilkan total di tengah dan efek 'pop out' saat hover
                plotOptions: {
                    pie: {
                        expandOnClick: true,
                        donut: {
