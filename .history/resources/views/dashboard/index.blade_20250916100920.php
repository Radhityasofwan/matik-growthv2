...
@php
    ...
    // fallback aman jika controller belum sempat kirim
    $stats = $stats ?? [
        'totalLeads' => 0,
        'leadsThisWeek' => 0,
        'leadsPreviousWeek' => 0,
        'messagesSentLast7Days' => 0,
        'messagesSentPrevious7Days' => 0,
        'replyRate' => 0.0,
...
@endphp
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start gap-4 mb-8">
        ...
        <div class="flex items-center gap-2">
            <a href="{{ route('leads.index') }}" class="btn btn-outline btn-primary">
                    ...
                    <div class="flex items-start">
                        <div class="avatar mr-4">
                            <div class="w-10 h-10 rounded-full bg-secondary text-primary flex items-center justify-center">
                                @if(Str::contains(Str::lower($activity->description ?? ''), 'import'))
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="16 16 12 12 8 16"></polyline><line x1="12" y1="12" x2="12" y2="21"></line><path d="M20 21H4"></path><path d="M4 3h16v4H4z"></path></svg>
                                @elseif(Str::contains(Str::lower($activity->description ?? ''), ['broadcast','kirim','wa']))
                                ...
                                @else
                                    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16 16l5-5-5-5"/><path d="M3 12h12"/></svg>
                                @endif
                            </div>
                        </div>
                        <div class="min-w-0">
    ...
    {{-- (Opsional) blok ringkas untuk ViewModel yang sudah ada --}}
    @if(isset($todayTasks) && method_exists($todayTasks, 'items') && count($todayTasks->items() ?? []) > 0)
        <div class="mt-8 card bg-base-100 shadow-md border border-base-300/50">
            <div class="card-body">
                <h2 class="card-title">Tugas Hari Ini</h2>
                <ul class="list-disc pl-5 text-sm">
                    @foreach($todayTasks->items() as $t)
                        <li>{{ $t['title'] ?? $t->title ?? 'Tugas' }} <span class="text-neutral/60">— {{ $t['due'] ?? $t->due ?? '' }}</span></li>
                    @endforeach
                </ul>
            </div>
        </div>
            ...
            init() {
                if (!chartData || !chartData.categories || !chartData.leads || !chartData.messages) {
                    document.querySelector("#main-chart").innerHTML =
                        '<div class="flex items-center justify-center h-full text-neutral/60"><p>Data chart tidak tersedia.</p></div>';
                    return;
                }
                const options = {
                    series: [
                    ...
                    legend: { position: 'top', horizontalAlign: 'right', markers: { radius: 12 }, labels: { colors: '#64748B' } }
                };
