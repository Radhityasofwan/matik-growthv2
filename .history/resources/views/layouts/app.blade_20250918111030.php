...
<html
  ...
  x-data="{
    ...
    // OPTIMIZED: Greeting logic now includes a dynamic icon based on the time of day
    get greeting() {
        ...
        const icons = {
            ...
            afternoon: '<svg xmlns=\'http://www.w3.org/2000/svg\' class=\'h-6 w-6\' fill=\'none\' viewBox=\'0 0 24 24\' stroke=\'currentColor\' stroke-width=\'2\'><path d=\'M12 10V2\'/><path d=\'m4.93 10.93 1.41 1.41\'/><path d=\'M2 18h2\'/><path d=\'M20 18h2\'/><path d=\'m19.07 10.93-1.41 1.41\'/><path d=\'M22 22H2\'/><path d=\'m16 6-4 4-4-4\'/></svg>',
            night: '<svg xmlns=\'http://www.w3.org/2000/svg\' class=\'h-6 w-6\' fill=\'none\' viewBox=\'0 0 24 24\' stroke=\'currentColor\' stroke-width=\'2\'><path d=\'M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z\'/></svg>'
        };
        if (h >= 5 && h < 11) return { text: 'Selamat Pagi', icon: icons.morning };
        if (h >= 11 && h < 15) return { text: 'Selamat Siang', icon: icons.morning };
        if (h >= 15 && h < 18) return { text: 'Selamat Sore', icon: icons.afternoon };
        return { text: 'Selamat Malam', icon: icons.night };
    }
  }"
...
>
  ...
  <div class="flex min-h-screen">
    ...
    <div class="hidden lg:block transition-all duration-300 ease-in-out" :class="$store.ui.sidebarHover ? 'w-64' : 'w-20'"></div>
    ...
    <div class="flex flex-1 flex-col contain-layout">
      ...
      <main id="main-content" class="flex-1 overflow-x-hidden overflow-y-auto p-4 sm:p-6 lg:p-8 contain-paint">
        <div class="mx-auto w-full max-w-7xl" data-aos="fade-up">
          @yield('content')
        </div>
      </main>
    </div>
