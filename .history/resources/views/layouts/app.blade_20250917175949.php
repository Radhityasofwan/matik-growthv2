{{-- resources/views/layouts/app.blade.php — FINAL STABLE --}}
<!DOCTYPE html>
...
<head>
    ...
    {{-- Preload theme before paint to avoid FOUC --}}
    <script>
      (function () {
...
<body
  ...
  x-data="{
      ...
      setTheme(t) {
        ...
        if (this.darkThemes.includes(t)) root.classList.add('dark'); else root.classList.remove('dark');
        // event untuk komponen lain (chart, dll)
  ...
  @keydown.k.window.prevent="
    // Cmd/Ctrl+K membuka search
    (e => { if (e.metaKey || e.ctrlKey) { isSearchOpen = true; $nextTick(() => $refs.searchInput?.focus()) } })(event)
  "
>
  ...
  >
    <div class="modal-box bg-base-100">
      ...
      <div class="mt-3 text-xs text-base-content/60">
        Tekan <kbd class="kbd kbd-xs">Esc</kbd> untuk menutup •
        Pintasan <kbd class="kbd kbd-xs">⌘</kbd>/<kbd class="kbd kbd-xs">Ctrl</kbd> + <kbd class="kbd kbd-xs">K</kbd>
      </div>
    </div>
