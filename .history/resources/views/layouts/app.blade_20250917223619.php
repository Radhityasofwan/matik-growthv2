...
<head>
  ...
  <script>
    // Paint theme ASAP sebelum CSS di-load (tanpa Alpine).
    (function(){
    ...
    // Inisialisasi Alpine Store (dipanggil saat Alpine init).
    document.addEventListener('alpine:init', () => {
      const themes = ['softblue','light','dark','cupcake','bumblebee','emerald','corporate','synthwave','retro','cyberpunk','valentine','halloween','garden','forest','aqua','lofi','pastel','fantasy','wireframe','black','luxury','dracula','cmyk','autumn','business','acid','lemonade','night','coffee','winter'];
      ...
      Alpine.store('ui', {
        ...
        isSearchOpen: false,       // modal pencarian
      ...
      // Harden mobile/desktop switch & gestures
      const syncMode = () => { if (window.innerWidth >= 1024) Alpine.store('ui').isMobileMenuOpen = false; };
      window.addEventListener('resize', syncMode);
      document.addEventListener('keydown', (e)=>{ if(e.key==='Escape') Alpine.store('ui').isMobileMenuOpen=false; });

      // Tutup drawer saat navigasi link di sidebar
      document.addEventListener('sidebar:navigated', ()=>{ Alpine.store('ui').isMobileMenuOpen=false; });
      ...
      document.addEventListener('touchend', (e)=>{
        ...
        if(startX!=null && endX!=null && (startX - endX) > 50) Alpine.store('ui').isMobileMenuOpen=false;
        startX = null;
      }, {passive:true});
    });
  </script>
...
>
  <div class="flex min-h-screen">
    {{-- SIDEBAR (terpisah: mobile & desktop) --}}
    ...
    <div
      ...
      :class="{
        // desktop: margin-left mengikuti lebar sidebar (collapsed 5rem vs expanded 16rem)
        'lg:ml-20': !$store.ui.sidebarHover,
        'lg:ml-64':  $store.ui.sidebarHover
      }"
    >
  ...
  {{-- SEARCH MODAL (pakai Alpine Store) --}}
  <dialog
    x-ref="searchModal"
