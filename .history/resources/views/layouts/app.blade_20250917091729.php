...
<html
    ...
    {{-- State Management Tema Global (Alpine + localStorage) --}}
    x-data="{
...
>
    ...
    <div class="flex min-h-screen">
        ...
        <aside
            class="z-40"
            :class="{
                'fixed inset-y-0 left-0 w-64 translate-x-0 lg:translate-x-0 transition-transform duration-300 ease-in-out': true,
                'lg:block': true,
                // Mobile: show/hide via slide
                '-translate-x-full': !isMobileMenuOpen && window.innerWidth <= 1024,
                'translate-x-0': isMobileMenuOpen && window.innerWidth <= 1024,
                // Desktop: collapsed width visual tetap dikontrol di container utama
            }"
            role="navigation"
            aria-label="Sidebar"
        ...
        <div
            ...
            :class="{
                // Desktop spacing terhadap sidebar
                'lg:ml-64': isSidebarExpanded,
                'lg:ml-20': !isSidebarExpanded,
            }"
        >
            {{-- Navbar (berisi tombol toggle yang memanggil method di atas) --}}
            <div
                x-data
