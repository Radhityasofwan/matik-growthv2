...
>
  ...
  <body
    ...
    x-data="{
      isSidebarExpanded:false,   // desktop diatur hover (bukan toggle)
      isMobileMenuOpen:false,    // mobile diatur tombol navbar
      isSearchOpen:false,
      greet(){
    ...
    x-init="window.addEventListener('resize',()=>{ if(window.innerWidth>=1024) isMobileMenuOpen=false })"
    @keydown.escape.window="isMobileMenuOpen=false; isSearchOpen=false"
    :class="{'overflow-hidden': isMobileMenuOpen}"
  >
    <div class="flex min-h-screen">
      @include('partials.sidebar')

      {{-- konten ikut melebar saat sidebar hover (desktop) via peer --}}
      <div class="flex flex-1 flex-col transition-all duration-300 ease-in-out lg:ml-20 lg:peer-hover:ml-64 lg:peer-focus-within:ml-64">
    ...
    >
      <div class="modal-box bg-base-100">
        ...
        <label class="input input-bordered flex items-center gap-2">
          ...
          <input x-ref="searchInput" type="text" class="grow border-0 bg-transparent focus:outline-none" placeholder="Ketik untuk mencari…" aria-label="Cari"/>
        </label>
