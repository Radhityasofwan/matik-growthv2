...
@php
    ...
    // Helper kecil untuk mengganti placeholder -> nilai contoh (regex sama dengan controller)
                        ...
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-700 text-sm">
                                <div class="flex items-center gap-2">
                                    ...
                                    <a href="#preview_template_{{ $template->id }}" class="btn btn-ghost btn-xs" title="Preview" aria-label="Preview">
                                        {{-- eye icon --}}
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-700 text-sm text-right">
                                <div class="flex items-center gap-3 justify-end">
                                    ...
                                    <form action="{{ route('whatsapp.templates.destroy', $template) }}" method="POST" onsubmit="return confirm('Delete this template?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                ...
                <div>
                    ...
                    <div class="text-xs text-gray-500 mt-2 space-y-1">
                        ...
                        <ul class="list-disc ml-4">
                            <li>Tulis placeholder dengan kurung ganda: <code>&#123;&#123;name&#125;&#125;</code>, <code>&#123;&#123;date&#125;&#125;</code></li>
                            <li>Boleh berisi huruf, angka, titik, dash, underscore. Contoh: <code>&#123;&#123;user.email&#125;&#125;</code>, <code>&#123;&#123;first-name&#125;&#125;</code></li>
                            <li>Variabel akan otomatis terdeteksi dan disimpan di kolom <em>variables</em>.</li>
                        </ul>
                    </div>
                </div>
...
@php
    $filled = renderFilled($tpl->body, $tpl->variables, $sampleMap);
@endphp
<div id="preview_template_{{ $tpl->id }}" class="modal">
    <div class="modal-box w-11/12 max-w-2xl">
        <a href="#" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</a>
        <h3 class="font-bold text-lg">Preview: {{ $tpl->name }}</h3>
        <div class="mt-4 grid gap-3">
            ...
            <div class="p-3 rounded-lg bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100">
                ...
                <div class="whitespace-pre-wrap break-words">{{ $tpl->body }}</div>
            </div>
        </div>
        <div class="modal-action">
...
{{-- Edit Modals --}}
@foreach ($templates as $tpl)
<div id="edit_template_{{ $tpl->id }}" class="modal">
    <div class="modal-box w-11/12 max-w-2xl">
        <form action="{{ route('whatsapp.templates.update', $tpl) }}" method="POST">
            @csrf
            @method('PATCH')
            <a href="#" class="btn btn-sm btn-circle btn-ghost absolute right-2 top-2">✕</a>
            ...
            <div class="mt-4 space-y-4">
                <div>
                    <label class="label"><span class="label-text">Name</span></label>
                    <input type="text" name="name" value="{{ old('name', $tpl->name) }}" class="input input-bordered w-full" required />
                </div>
                <div>
                    <label class="label"><span class="label-text">Body</span></label>
                    <textarea name="body" rows="5" class="textarea textarea-bordered w-full" required>{{ old('body', $tpl->body) }}</textarea>
                    <div class="text-xs text-gray-500 mt-2 space-y-1">
                        <div class="font-semibold">Panduan variabel:</div>
                        <ul class="list-disc ml-4">
                            <li>Gunakan format: <code>&#123;&#123;variable&#125;&#125;</code> (mis. <code>&#123;&#123;name&#125;&#125;</code>, <code>&#123;&#123;date&#125;&#125;</code>)</li>
                            <li>Variabel yang terdeteksi saat ini:
                ...
                <div class="form-control">
                    <label class="label cursor-pointer justify-start gap-3">
                        <input type="checkbox" name="is_active" value="1" class="toggle toggle-primary" @checked(old('is_active', $tpl->is_active)) />
                        <span class="label-text">Active</span>
                    </label>
                </div>
            </div>
