            ...
            <table class="min-w-full leading-normal">
                ...
                <tbody class="bg-white dark:bg-gray-800">
                    @forelse ($templates as $template)
                        @php
                            $filledPreview = renderFilled($template->body, $template->variables, $sampleMap);
                        @endphp
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b border-gray-200 dark:border-gray-700 text-sm text-right">
                                <div class="flex items-center gap-3 justify-end">
                                    ...
                                    <form action="{{ route('whatsapp.templates.destroy', $template) }}"
...
<div id="create_template_modal" class="modal">
    <div class="modal-box w-11/12 max-w-2xl">
        <form action="{{ route('whatsapp.templates.store') }}#create_template_modal" method="POST">
            @csrf
...
<div id="edit_template_{{ $tpl->id }}" class="modal">
    <div class="modal-box w-11/12 max-w-2xl">
        <form action="{{ route('whatsapp.templates.update', $tpl) }}#edit_template_{{ $tpl->id }}" method="POST">
            @csrf
            @method('PATCH')
            <input type="hidden" name="_form" value="edit">
            <input type="hidden" name="_id" value="{{ $tpl->id }}">
            ...
            <div class="mt-4 space-y-4">
                <div>
                    ...
                    <input
                        type="text"
                        name="name"
                        value="{{ old('_form')==='edit' && (int)old('_id')===(int)$tpl->id ? old('name', $tpl->name) : $tpl->name }}"
                        class="input input-bordered w-full" required />
                    @error('name')
                        @if(old('_form')==='edit' && (int)old('_id')===(int)$tpl->id)
                ...
                <div>
                    <label class="label"><span class="label-text">Body</span></label>
                    <textarea name="body" rows="5" class="textarea textarea-bordered w-full" required>{{ old('_form')==='edit' && (int)old('_id')===(int)$tpl->id ? old('body', $tpl->body) : $tpl->body }}</textarea>
                    @error('body')
                        @if(old('_form')==='edit' && (int)old('_id')===(int)$tpl->id)
                            <div class="text-error text-xs mt-1">{{ $message }}</div>
                        @endif
                ...
                <div class="form-control">
                    <label class="label cursor-pointer justify-start gap-3">
                        <input type="checkbox" name="is_active" value="1" class="toggle toggle-primary"
                            @checked(old('_form')==='edit' && (int)old('_id')===(int)$tpl->id ? old('is_active', $tpl->is_active) : $tpl->is_active) />
                        <span class="label-text">Active</span>
                    </label>
                </div>
            </div>
