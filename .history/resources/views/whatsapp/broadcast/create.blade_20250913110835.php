{{-- resources/views/whatsapp/broadcast/create.blade.php --}}
@extends('layouts.app')
...
@php
    ...
    // Ambil daftar sender secara aman (fallback bila controller tidak mengoper)
    $senders = $senders
    ...
    // Pastikan $templates minimal punya id & name (sudah diaman-kan di Controller)
@endphp
...
<div class="container mx-auto px-6 py-8" x-data="broadcastPage()">
    ...
    @if ($errors->any())
        <div class="alert alert-error shadow mb-6">
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m7 10a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <span>
                        ...
                        <div>
                            ...
                            <select class="select select-bordered w-full"
                                ...
                                <template x-if="senders.length === 0">
                                    <option disabled>Belum ada sender aktif</option>
                                </template>
                                <template x-for="s in senders" :key="s.id">
                                    <option :value="s.id" x-text="senderLabel(s)"></option>
                                </template>
                            </select>
                            <p class="text-xs text-gray-500 mt-1" x-show="defaultSenderId">Default sender akan ditandai ✨</p>
                        </div>
...
628123456789 | Budi"
                    ...
                    <div class="mt-5" x-show="form.mode === 'template'">
                        ...
                        <p class="text-xs text-gray-500 mt-1">Preview isi template tidak tersedia (tergantung pihak WA). Pastikan params sesuai dengan template yang telah disetujui.</p>
                    </div>
...
function broadcastPage(){
    return {
        ...
        defaultSenderId: @json(optional($senders->firstWhere('is_default', true))->id),
        form: {
            sender_id: '',
            mode: 'custom',
            recipients: '',
        ...
        updatePreview(){
            this.preview.items = this.parseRecipients();
        },

        resetForm(){
            this.form = { sender_id:'', mode:'custom', recipients:'', message:'', template_id:'', params_json:'' };
            this.preview.items = [];
        },
        ...
        refreshSenders(){ window.location.reload(); },
