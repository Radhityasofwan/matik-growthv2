        ...
        <div class="card-body">
            <form method="POST" action="{{ route('whatsapp.broadcast.store') }}">
                ...
                <div class="mb-5">
                    ...
                    <p class="text-xs text-gray-500 mt-1">Pilih nomor pengirim. Default ditandai ⭐.</p>
                </div>
                ...
                <div class="mb-5" x-show="mode === 'custom'">
                    ...
                    <textarea name="message" class="textarea textarea-bordered w-full h-32" placeholder="Gunakan {{'{{name}}'}} untuk nama penerima.">{{ old('message') }}</textarea>
                    <p class="text-xs text-gray-500 mt-1">Placeholder: <code>{{'{{name}}'}}</code>, <code>{{'{{nama}}'}}</code>, <code>{{'{{nama_pelanggan}}'}}</code></p>
                </div>
                ...
                <div class="mb-5" x-show="mode === 'template'">
                    ...
                    <select name="template_id" class="input input-bordered w-full">
                        ...
                        @foreach($templates as $tpl)
                            <option value="{{ $tpl->id }}" @selected(old('template_id') == $tpl->id)>{{ $tpl->name ?? ('Template #'.$tpl->id) }}</option>
                        @endforeach
                    </select>
