...
628123456789 | Budi" required>{{ old('recipients') }}</textarea>
                        ...
                        <div id="boxTemplate" class="mt-5 hidden">
                            ...
                            <textarea name="params_json" class="textarea textarea-bordered w-full" placeholder='Contoh: {"code":"AB-123","promo":"SEPT25"}'>{{ old('params_json') }}</textarea>
                            <p class="text-xs text-gray-500 mt-1">Variabel sesuai yang terdeteksi di template (misal: <code>{{ '{' }}{{ 'name' }}{{ '}' }}</code>, <code>{{ '{' }}{{ 'code' }}{{ '}' }}</code>, dll).</p>
                        </div>
...
{{-- Toggle section custom/template + preview sederhana --}}
<script>
(function(){
  ...
  const paramsJson = document.querySelector('textarea[name="params_json"]');
  ...
  [recipients, message, paramsJson].forEach(el => el && el.addEventListener('input', renderPreview));
  ...
  function renderPreview(){
    ...
    if (modeCustom.checked) {
      ...
      list.forEach(r=>{
        let t = raw;
        t = t.replaceAll('{{name}}', r.name).replaceAll('@{{name}}', r.name)
             .replaceAll('{{nama}}', r.name).replaceAll('{{nama_pelanggan}}', r.name);
      ...
      list.forEach(r=>{
        ...
        // Sekadar preview sederhana untuk {{name}} dkk
        t = t.replaceAll('{{name}}', ctx.name).replaceAll('@{{name}}', ctx.name)
             .replaceAll('{{nama}}', ctx.name).replaceAll('{{nama_pelanggan}}', ctx.name);
        txt += `<div class="p-2 rounded border"><div class="opacity-60">→ ${r.phone} (${r.name})</div><div>${escapeHtml(t)}</div></div>`;
      });
    }
