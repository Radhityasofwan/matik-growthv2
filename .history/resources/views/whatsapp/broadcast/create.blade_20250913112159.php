...
@php
    ...
    // fallback: isi dropdown saat first render (nanti bisa di-refresh via AJAX)
    $senders = $senders
        ?? \App\Models\WahaSender::query()
            ...
            ->get(['id','name','number','session','is_active','is_default']);
@endphp
...
function broadcastPage(){
    return {
        ...
        // Refresh (kompatibel dgn WahaSenderController@index -> JSON: {data:[...]})
        refreshSenders(){
        ...
        submitSender(){
            ...
            fetch(el.action, {
                method: (this.formSender.id ? 'POST' : 'POST'), // pakai _method=PUT saat edit
                headers: { 'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json' },
                body: fd
            })
            .then(r => r.json())
            .then(() => { this.refreshSenders(); this.resetSenderForm(); })
        ...
        routes: {
            ...
            update(id){ return "{{ url('waha-senders') }}/"+id; }, // {wahaSender}
