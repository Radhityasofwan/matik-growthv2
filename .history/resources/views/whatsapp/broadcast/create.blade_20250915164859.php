...
<script>
(async function () {
  // KUMPULKAN id yang ingin dicek (contoh ambil dari variabel "list")
  const ids = (window.list || []).map(x => x.id).filter(Boolean);
  if (!ids.length) return;

  try {
    const url = `{{ route('waha.sessions.statusBatch') }}?ids=${encodeURIComponent(ids.join(','))}`;
    const rs  = await fetch(url, { headers: { 'Accept': 'application/json' } });

    if (!rs.ok) throw new Error(`HTTP ${rs.status}`);
    const js = await rs.json();

    const map = {};
    (js.data || []).forEach(it => { map[it.id] = it; });

    // lakukan update tampilan sesuai kebutuhanmu
    (window.list || []).forEach(it => {
      const m = map[it.id] || {};
      it.__status  = m.state || null;
      it.__connected = m.connected;
    });
  } catch (e) {
    // diamkan/console bila perlu
    console.warn('statusBatch error', e);
  }
})();
</script>
@endsection
