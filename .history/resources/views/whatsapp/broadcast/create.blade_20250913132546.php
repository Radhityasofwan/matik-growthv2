...
628123456789 | Budi" required>{{ old('recipients') }}</textarea>
                        ...
                        {{-- Template (disembunyikan saat awal) --}}
...
{{-- Fallback JS (tanpa Alpine) + Refresh Sender (dengan status WAHA) --}}
<script>
(function () {
    // Toggle kolom Custom/Template (fallback JS murni)
    ...
    // in case of server-side error restore old('mode')
    @if(old('mode') === 'template')
        document.getElementById('mode_template').checked = true;
    @endif
    syncMode();

    // Refresh Sender + tampilkan status WAHA
