...
function broadcastPage() {
    return {
        ...
        buildSenderLabel(s) {
            let base = s.name ?? s.number ?? s.session ?? ('Sender #' + s.id);
            if (s.is_default) base += ' — Default';
            if (s.__status)   base += ` (${s.__status})`;
            return base;
        },
                ...
                if (ids.length) {
                    try {
                        ...
                        if (rs.ok) {
                            ...
                            const map = {};
                            (js.data || []).forEach(it => map[it.id] = it.status || null);
