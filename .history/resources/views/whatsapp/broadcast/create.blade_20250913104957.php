...
628123456789 | Budi"
                    ...
                    <div class="mt-8 flex flex-col-reverse md:flex-row items-stretch md:items-center gap-3 md:justify-end">
                        ...
                        >
                            {{-- Fallback label (muncul walau Alpine gagal) --}}
                            <span class="inline" x-show="!submitting">Kirim Broadcast</span>
                            <span class="inline" x-show="submitting">Mengirim...</span>
                        </button>
                    </div>
                </form>
            </div>
                ...
                <div class="lg:col-span-2">
                    <div class="p-4 rounded bg-gray-50 dark:bg-gray-700">
                        ...
                        <form @submit.prevent="submitSender">
                            <div class="space-y-3">
                                <div><label class="label">Nama</label><input type="text" class="input input-bordered w-full" x-model="form.name" required></div>
                                <div><label class="label">Nomor</label><input type="text" class="input input-bordered w-full" x-model="form.number" placeholder="628xxxx" required></div>
                                <div><label class="label">Session</label><input type="text" class="input input-bordered w-full font-mono text-sm" x-model="form.session" required></div>
                                <div><label class="label">Deskripsi</label><input type="text" class="input input-bordered w-full" x-model="form.description"></div>
                            ...
                            <div class="mt-4 flex items-center justify-end gap-2">
                                ...
                                <button type="submit" class="btn btn-primary" :disabled="savingSender">
                                    <span x-show="!savingSender">Simpan</span>
                                    <span x-show="savingSender">Menyimpan...</span>
                                </button>
                            </div>
                        </form>
                    </div>
...
<script>
(function(){
  // Factory komponen — terpasang global & via alpine:init agar selalu terdeteksi
  function factory() {
    return {
        // ---- Broadcast state ----
        mode: 'custom',
        ...
        get canSend(){
            ...
            const hasSender = !!(sel && sel.value);
            if (this.mode === 'custom') return hasSender && this.parsed.length > 0 && (this.message || '').trim().length > 0;
            return hasSender && this.parsed.length > 0;
        },
        ...
        resetPreview() { this.message = ''; this.recipientsRaw = ''; this.parseRecipients(); },

        async onSubmit(e) {
            if (!this.canSend) { e.preventDefault(); alert('Lengkapi sender, penerima, dan isi pesan.'); return; }
            this.submitting = true; // submit standar
        },
        ...
        async loadSenders() {
            ...
            try {
                const url = this.routes.index + '?json=1'; // fallback jika server tidak cek Accept header
  ...
  // Pasang global + registrasi ke Alpine jika tersedia
  window.broadcastPage = factory;
  document.addEventListener('alpine:init', () => {
      if (window.Alpine && Alpine.data) Alpine.data('broadcastPage', factory);
  });
})();
</script>
@endsection
