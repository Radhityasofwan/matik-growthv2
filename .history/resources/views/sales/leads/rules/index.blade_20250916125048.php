        ...
        <form action="{{ route('lead-follow-up-rules.store') }}" method="POST" class="grid grid-cols-1 lg:grid-cols-12 gap-3 items-end">
            ...
            <div class="lg:col-span-3">
                ...
                <select name="lead_id" class="select select-bordered w-full">
                    ...
                    @foreach($leads as $ld)
                        <option value="{{ $ld->id }}">{{ $ld->name ?: ($ld->store_name ?: 'Lead #'.$ld->id) }} — {{ $ld->email }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="lg:col-span-12 flex items-center gap-4 mt-2">
                {{-- hidden supaya selalu kirim 0/1 --}}
                <input type="hidden" name="is_active" value="0">
                <label class="label cursor-pointer">
                    <span class="label-text mr-3">Aktif</span>
                    <input type="checkbox" name="is_active" value="1" class="toggle toggle-success" checked>
                </label>
                                ...
                                <div class="grid grid-cols-1 lg:grid-cols-12 gap-3">
                                    <div class="lg:col-span-3">
                                        ...
                                        <select name="lead_id" class="select select-bordered w-full">
                                            ...
                                            @foreach($leads as $ld)
                                                <option value="{{ $ld->id }}" @selected($rule->lead_id === $ld->id)>{{ $ld->name ?: ($ld->store_name ?: 'Lead #'.$ld->id) }} — {{ $ld->email }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    ...
                                    <div class="lg:col-span-12 flex items-center gap-4 mt-2">
                                        ...
                                        <label class="label cursor-pointer">
                                            <span class="label-text mr-3">Aktif</span>
                                            <input type="checkbox" name="is_active" value="1" class="toggle toggle-success" @checked($rule->is_active)>
                                        </label>
