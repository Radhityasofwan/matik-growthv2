    ...
    @if ($errors->any())
        <div class="alert alert-error shadow-lg mb-6">
            <div>
                ...
                <span><strong>Terdapat kesalahan!</strong> Mohon periksa kembali data yang Anda masukkan.</span>
            </div>
        </div>
    @endif
            ...
            <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                <table class="min-w-full leading-normal">
                    ...
                    <tbody class="bg-white dark:bg-gray-800">
                        ...
                        @empty
                        <tr>
                            <td colspan="5" class="px-5 py-10 text-center text-gray-500">No leads found.</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
...
<!-- Create/Edit Modals (omitted for brevity, assume they are the same as before but with a 'phone' input field) -->
<!-- Create Lead Modal -->
        ...
        <form action="{{ route('leads.store') }}" method="POST">
            ...
            <div class="mt-4 space-y-4">
                ...
                <div>
                    ...
                    <input type="email" name="email" placeholder="Enter email address" value="{{ old('email') }}" class="input input-bordered w-full" required />
                </div>
                <div>
                ...
                <div>
                    ...
                    <select name="status" class="select select-bordered w-full" required>
                        <option value="trial">Trial</option> <option value="active">Active</option> <option value="converted">Converted</option> <option value="churn">Churn</option>
                    </select>
                </div>
                <div>
                    ...
                    <select name="owner_id" class="select select-bordered w-full" required>
                        @forelse($users as $user) <option value="{{ $user->id }}">{{ $user->name }}</option> @empty <option disabled>No users</option> @endforelse
                    </select>
                </div>
            </div>
...
<div id="whatsapp_modal" class="modal">
    <div class="modal-box w-11/12 max-w-2xl">
        ...
        <div class="modal-action mt-6">
            <a href="#" class="btn btn-ghost">Cancel</a>
            <a id="wa-send-button" href="#" target="_blank" class="btn btn-success" disabled>Send via WhatsApp</a>
        </div>
    </div>
    <a href="#" class="modal-backdrop">Close</a>
</div>
...
<script>
    ...
    waTemplateSelector.addEventListener('change', function() {
        ...
        // --- PERUBAHAN DI SINI ---
        // Mengganti variabel seperti {{name}} atau {{nama}} dengan nama lead
