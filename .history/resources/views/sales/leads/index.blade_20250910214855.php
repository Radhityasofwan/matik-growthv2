...
<div class="container mx-auto px-6 py-8">
    ...
    @if (session('success'))
        <div class="alert alert-success shadow-lg mb-6">
            <div>
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" class="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    ...
    @if ($errors->any())
        <div class="alert alert-error shadow-lg mb-6">
            <div>
                <svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" class="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m2 2l2 2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
            ...
            <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                <table class="min-w-full leading-normal">
                     <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-5 py-3 border-b-2"><input type="checkbox" id="select-all-checkbox" class="checkbox checkbox-sm" /></th>
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Name</th>
                            ...
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Created At</th>
                            <th class="px-5 py-3 border-b-2">Actions</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-gray-800">
                        @forelse ($leads as $lead)
                        <tr id="lead-row-{{ $lead->id }}" data-phone="{{ $lead->phone }}" data-name="{{ $lead->name }}">
                            <td class="px-5 py-5 border-b"><input type="checkbox" class="checkbox checkbox-sm lead-checkbox" value="{{ $lead->id }}" data-phone="{{ $lead->phone }}" data-name="{{ $lead->name }}"></td>
                            <td class="px-5 py-5 border-b text-sm">
                                <p class="font-semibold">{{ $lead->name }}</p>
                                <p class="text-gray-600">{{ $lead->email }}</p>
                                <p class="text-gray-500 mt-1">{{ $lead->phone ?? 'No phone' }}</p>
                            </td>
                            <td class="px-5 py-5 border-b text-sm"><span class="badge @switch($lead->status) @case('trial') badge-info @break @case('active') badge-success @break @case('converted') badge-accent @break @case('churn') badge-error @break @endswitch">{{ ucfirst($lead->status) }}</span></td>
                            <td class="px-5 py-5 border-b text-sm">{{ $lead->owner?->name ?? 'Unassigned' }}</td>
                            <td class="px-5 py-5 border-b text-sm">{{ $lead->created_at->format('M d, Y') }}</td>
                            <td class="px-5 py-5 border-b text-sm text-right">
                                <div class="flex items-center justify-end space-x-3">
                                    @if($lead->phone)<a href="#whatsapp_modal" class="text-green-500 hover:text-green-700" onclick="openWhatsAppModal({{ $lead->id }})"><svg xmlns="[http://www.w3.org/2000/svg](http://www.w3.org/2000/svg)" width="20" height="20" fill="currentColor" class="bi bi-whatsapp" viewBox="0 0 16 16"><path d="M13.601 2.326A7.854 7.854 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.933 7.933 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.898 7.898 0 0 0 13.6 2.326zM7.994 14.521a6.573 6.573 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.557 6.557 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592zm3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.729.729 0 0 0-.529.247c-.182.198-.691.677-.691 1.654 0 .977.71 1.916.81 2.049.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232z"/></svg></a>@endif
...
<!-- Other Modals (Single and Bulk WA) -->
<div id="whatsapp_modal" class="modal">...</div>
<div id="bulk_whatsapp_modal" class="modal">...</div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add event listener to all status selectors
    document.querySelectorAll('.status-selector').forEach(selector => {
        selector.addEventListener('change', function() {
            const leadId = this.dataset.leadId;
            const selectedStatus = this.value;
            toggleSubscriptionForm(leadId, selectedStatus);
        });
    });

    // --- Other Scripts (Single WA, Bulk Actions) ---
