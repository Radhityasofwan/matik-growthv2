    ...
    @if ($errors->any())
        <div class="alert alert-error shadow-lg mb-6">
            <div>
                ...
                <span><strong>Terdapat kesalahan!</strong><ul class="list-disc ml-5">@foreach($errors->all() as $e)<li>{{ $e }}</li>@endforeach</ul></span>
            </div>
        </div>
    @endif
                ...
                <table class="min-w-full leading-normal">
                    ...
                    @foreach($leads as $lead)
                        <tr id="lead-row-{{ $lead->id }}" data-phone="{{ $lead->phone }}" data-name="{{ $lead->name }}">
                            <td class="px-4 py-4 border-b"><input type="checkbox" class="checkbox checkbox-sm lead-checkbox" value="{{ $lead->id }}" data-phone="{{ $lead->phone }}" data-name="{{ $lead->name }}"></td>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.openWhatsAppModal = function(leadId) {
        ...
        waTplSel.selectedIndex = 0;
        waPrev.value = '';
        waSend.classList.add('btn-disabled');
    }
    if (waTplSel) waTplSel.addEventListener('change', function(){
        ...
        const msg = this.value.replace(/\{\{name\}\}/g, name).replace(/\{\{nama_pelanggan\}\}/g, name);
        waPrev.value = msg;
        waSend.href = `https://wa.me/${phone}?text=${encodeURIComponent(msg)}`;
        waSend.classList.remove('btn-disabled');
    });
    ...
    if (bulkTplSel) bulkTplSel.addEventListener('change', function(){
        bulkPrev.value = this.value || '';
        const selected = document.querySelectorAll('.lead-checkbox:checked');
        bulkSend.classList.toggle('btn-disabled', !this.value || selected.length===0);
    });
