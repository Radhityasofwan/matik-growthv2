                ...
                <div id="task-{{ $task->id }}" class="p-4 bg-white dark:bg-gray-900 rounded-lg shadow cursor-grab active:cursor-grabbing">
                    ...
                    <div class="mt-4 flex justify-between items-center text-sm">
                        <div class="flex items-center space-x-2 text-gray-500 dark:text-gray-400">
                            ...
                            <span @if($task->due_date && $task->due_date->isPast() && $task->status != 'done') class="text-error font-semibold" @endif>{{ $task->due_date?->format('d M Y') ?? 'No due date' }}</span>
                        </div>
    ...
    <div class="modal-box w-11/12 max-w-2xl">
        <form id="edit_task_form" action="" method="POST">
            ...
            <div class="mt-4 space-y-4">
                ...
                <div><label class="label"><span class="label-text">Status</span></label><select id="edit_status" name="status" class="select select-bordered w-full" required><option value="open">Open</option><option value="in_progress">In Progress</option><option value="done">Done</option><option value="overdue">Overdue</option></select></div>
            </div>
...
async function openEditModal(taskId) {
    try {
        ...
        // Versi baru ini akan memeriksa apakah data berada di dalam properti 'task'
        // atau langsung di dalam respons. Ini membuatnya lebih stabil.
