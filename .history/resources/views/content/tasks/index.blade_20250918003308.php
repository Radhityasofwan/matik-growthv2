...
@section('content')
<style>
  /* ==== ADAPTIF DAISYUI: pakai oklch(var(--token)/alpha) ==== */

  /* Permukaan papan ala whiteboard, mengikuti primary/accent */
  .board-surface{
    border-radius: 1rem;
    background:
  ...
  /* Nada/tone per kolom: set --tone menjadi triple angka (var(--p), var(--s), dll) */
  ...
  .board-col{
    border-radius: .9rem;
    /* tint elegan mengikuti --tone dan base tokens */
    background:
      linear-gradient(180deg, oklch(var(--tone)/.08), transparent 55%),
  ...
  /* Kartu (strip warna di atas adaptif) */
  ...
  .task-bar-primary   { border-top: 4px solid oklch(var(--p)); }
  .task-bar-secondary { border-top: 4px solid oklch(var(--s)); }
  .task-bar-accent    { border-top: 4px solid oklch(var(--a)); }
...
@php
  ...
  // Palet variasi kartu (ikut theme karena token DaisyUI)
...
<div class="container mx-auto py-6 board-surface">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    ...
    <button type="button" class="btn btn-success text-success-content shadow"
  ...
  <div class="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
    @foreach ($statusTitles as $status => $title)
      <div class="board-col {{ $statusTones[$status] }}">
        <div class="p-4">
          ...
          <div id="{{ $status }}" class="kanban-column board-col-body space-y-4" data-status="{{ $status }}">
            @foreach ($tasks[$status] as $task)
              @php
                ...
                $taskColor = $task->color ?? $auto; // variatif + hormati preferensi task
              @endphp
                        ...
                        <div class="dropdown dropdown-end">
                          <label tabindex="0" class="btn btn-ghost btn-xs btn-circle">
                            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 5v.01M12 12v.01M12 19v.01M12 6a1 1 0 110-2 1 1 0 010 2z" />
                            </svg>
                          </label>
