...
<style>
  ...
  /* cegah drag pada kontrol UI */
  .no-drag, .no-drag * { touch-action: manipulation; user-select: none; }
</style>
@endpush
        ...
        <div class="card-body p-3">
          ...
          >
            @forelse ($tasks[$key] as $t)
              ...
              <div
                :id="ids.card({{ $t->id }})"
                class="kanban-card card bg-base-100 border border-base-300/50 shadow-sm hover:shadow-lg transition-all ring-1 {{ $pConf['ring'] }}"
                data-task="{{ $taskJson }}"
              >
                ...
                <div class="card-body p-3">
                  <div class="flex items-start gap-2">
                    ...
                    {{-- drag handle agar dropdown tidak ikut terseret --}}
                    <button class="drag-handle btn btn-ghost btn-xs mr-1" title="Drag">
                    ...
                    {{-- ACTION DROPDOWN (stabil + no-drag) --}}
                    <div x-data="{open:false}" class="dropdown dropdown-end no-drag">
                      <button type="button"
                      ...
                      <ul x-show="open"
                        ...
                        <li>
                          <a href="{{ route('tasks.edit', $t) }}"
                             @click.prevent="openEdit({{ $t->id }}); open=false">
                            Edit
                          </a>
                        </li>
                        <li>
                          <details>
                            ...
                            <ul class="bg-base-100">
                              @foreach ($columns as $c2)
                                @if ($c2['key'] !== $t->status)
                                  <li>
                                    <a href="{{ route('tasks.status.update', $t) }}"
                                       @click.prevent="moveTo({{ $t->id }}, '{{ $c2['key'] }}', true); open=false">
                                      → {{ $c2['label'] }}
                                    </a>
                                  </li>
                                @endif
                  ...
                  <div class="mt-3 flex items-center justify-between">
                    ...
                    @php $progress = $t->progress ?? null; @endphp
                    @if(!is_null($progress))
...
function taskBoard() {
  return {
    ...
    routes: {
      // GET /tasks/{id}/edit (Accept: application/json)
    ...
    countIn(key) { const el = document.getElementById(this.ids.col(key)); return el ? el.querySelectorAll('.kanban-card[data-task]').length : 0; },
    ...
    initSortable(key) {
      ...
      const makeSortable = () => {
        ...
        new Sortable(colEl, {
          ...
          handle: '.drag-handle',             // ⛳ hanya tombol grip yang bisa drag
          filter: '.no-drag, .dropdown, .dropdown *', // cegah drag saat klik UI
    ...
    async proceedConfirmWA(sendWA) {
      ...
      try {
        ...
        // sukses → Sortable sudah memindahkan card di DOM; tidak perlu apa-apa
      } catch (e) {
    ...
    applyFilters() {
      ...
      document.querySelectorAll('.kanban-card[data-task]').forEach(card => {
        try {
