...
@section('content')
<style>
  /* ===== Board look & feel (adaptif tema DaisyUI) ===== */
  .board-surface{
    border-radius: 1rem;
    background:
  ...
  /* Drag & Drop states (sinkron referensi) */
  ...
  /* Card visual; warna fleksibel via CSS variable --card-color */
...
@php
  ...
  function cssColor($val){
      ...
      if (preg_match('/^[a-z]+$/i', $v)) return $v;
      return '#3b82f6';
  }
@endphp
...
<div class="container mx-auto px-3 sm:px-4 lg:px-6 py-5 lg:py-8 board-surface">
  ...
  {{-- Board (scroll horizontal di mobile, grid di desktop) --}}
                          ...
                          <div class="avatar">
                            <div class="w-8 rounded-full ring ring-base-100">
                                @if ($task->assignee->avatar)
                                    <img src="{{ asset('storage/' . $task->assignee->avatar) }}" alt="Avatar of {{ $task->assignee->name }}" />
                                @else
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($task->assignee->name) }}&background=random&color=fff" alt="Avatar of {{ $task->assignee->name }}" />
                                @endif
                            </div>
                          </div>
                        </div>
  ...
  <div class="modal-box w-11/12 max-w-2xl">
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="mt-4 space-y-4" id="create_form">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <div class="form-control">
          <label class="label"><span class="label-text">Atau ketik (HEX/RGB/HSL/nama)</span></label>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  function normalizeHex3(h){ const r=h[1], g=h[2], b=h[3]; return `#${r}${r}${g}${g}${b}${b}` }
  // Sama seperti PHP cssColor() tapi di JS untuk update DOM realtime
  function colorToCSS(v){
    if(!v) return '#3b82f6';
    v = String(v).trim().toLowerCase();
    const map = {primary:'oklch(var(--p))',secondary:'oklch(var(--s))',accent:'oklch(var(--a))',
                 info:'oklch(var(--in))',success:'oklch(var(--su))',warning:'oklch(var(--wa))',
                 error:'oklch(var(--er))',neutral:'oklch(var(--n))'};
    if(map[v]) return map[v];
    if(/^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(v)) return v.length===4 ? normalizeHex3(v) : v;
  ...
  function prioBadgeClass(p){
    switch(p){case 'low':return'badge-info';case 'medium':return'badge-success';case 'high':return'badge-warning';case 'urgent':return'badge-error';default:return'badge-ghost'}
  }
  ...
  window.openEditModal = async function(taskId) {
    try {
      ...
      document.getElementById('edit_color_picker').value = hexMatch ? (hexMatch[0].length===4 ? normalizeHex3(hexMatch[0]) : hexMatch[0]) : '#3b82f6';
  ...
  /* ===== Submit edit via AJAX → update kartu di index tanpa reload ===== */
  const editForm = document.getElementById('edit_task_form');
  if (editForm) {
    editForm.addEventListener('submit', async (e) => {
      e.preventDefault();
      const form = e.target;
      const fd = new FormData(form);               // berisi _method PATCH
      try {
        const r = await fetch(form.action, {
          method: 'POST',                           // Laravel akan hormati _method=PATCH
          headers: { 'X-CSRF-TOKEN': csrf(), 'Accept': 'application/json' },
          body: fd
        });
        // Jika server tidak mengembalikan JSON, fallback reload
        let data = null;
        ...
        if (card) {
          // Warna
          card.style.setProperty('--card-color', colorToCSS(task.color || '#3b82f6'));
          ...
          if (prioEl) { prioEl.className = 'badge '+ prioBadgeClass(task.priority || 'medium') + ' badge-sm'; prioEl.textContent = (task.priority||'medium')[0].toUpperCase() + (task.priority||'medium').slice(1); }
