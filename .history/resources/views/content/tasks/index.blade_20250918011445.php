...
<style>
  ...
  /* Drag & Drop state (sinkron dengan referensi) */
        ...
        <div class="p-4 lg:p-5">
          ...
          <div id="{{ $status }}" class="kanban-column board-col-body space-y-4" data-status="{{ $status }}">
            @foreach ($tasks[$status] as $task)
              @php
                // Warna kartu fleksibel (HEX/RGB/HSL/nama), default fallback biru
                $rawColor = trim($task->color ?? '#3b82f6');
                $priorityBadge = prioBadge($task->priority);
              @endphp

              <div id="task-{{ $task->id }}"
                   class="card bg-base-100 shadow-md cursor-grab active:cursor-grabbing task-card"
                   style="--card-color: {{ $rawColor }};">
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // ====== DRAG & DROP (sesuai referensi, disinkronkan) ======
  ...
  // ====== ACTION: pindah status lewat menu ======
  ...
  window.openEditModal = async function(taskId) {
    try {
      ...
      // Jika hex valid, isi color picker
