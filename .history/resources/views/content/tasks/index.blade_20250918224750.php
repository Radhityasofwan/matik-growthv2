...
@push('styles')
<style>
  /* Agar kolom tidak meng-clipping dropdown/tooltip */
  .kanban-column { overflow: visible; isolation: isolate; }
  /* Skeleton card saat loading */
  .skeleton-card { @apply animate-pulse bg-base-200 rounded-xl h-24; }
  /* Teleport dropdown ke body dengan posisi absolut */
...
>
  ...
  {{-- Alerts --}}
  @if (session('success'))
  <div class="alert alert-success shadow mb-4" data-aos="fade-down">
    <span>{{ session('success') }}</span>
  </div>
  @endif
  @if ($errors->any())
  <div class="alert alert-error shadow mb-4" data-aos="fade-down">
    <div>
  ...
  {{-- Header --}}
  <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3" data-aos="fade-down">
    <div>
  ...
  {{-- Kanban --}}
  @php
    // Pastikan variabel tersedia dari controller
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-6">
    ...
    <div class="kanban-column card bg-base-100 border border-base-300/50 shadow-lg" data-aos="fade-up">
      <div class="card-body p-3">
        <div class="flex items-center justify-between px-1">
          <h3 class="font-semibold text-base-content">{{ $col['label'] }}</h3>
          <span class="badge badge-ghost"><span x-text="countIn('{{ $col['key'] }}')">{{ $tasks[$col['key']]->count() }}</span> item</span>
        </div>
        ...
        <div 
          ...
          :id="ids.col('{{ $col['key'] }}')"
          x-init="$nextTick(() => initSortable('{{ $col['key'] }}'))"
        >
          {{-- Server-rendered initial items (progressive enhancement) --}}
        @forelse (data_get($tasks, $col['key'], collect()) as $t)
          <div 
            :id="ids.card({{ $t->id }})"
            ...
            data-task='@json([
              ...
              "owners" => $t->owners->map(fn($u)=>['id'=>$u->id,'name'=>$u->name,'wa_number'=>$u->wa_number])->values(),
          ...
          >
            <div class="card-body p-3">
              <div class="flex items-start justify-between gap-2">
                ...
                <div class="relative" x-data="floatMenu()">
                  <button type="button" class="btn btn-ghost btn-xs" @click="toggle($el)">
                    opsi
                  </button>
                  <template x-teleport="body">
                    ...
                    >
                      ...
                      <li>
                        <a @click.prevent="askMove({{ $t->id }})">Pindah Ke…</a>
...
@push('scripts')
{{-- Sortable via Vite alias; jika belum diimport global, fallback CDN --}}
<script>
  // Fallback jika Sortable tidak terbundel via Vite
...
<script>
/**
 * Alpine component utama untuk Kanban board
 * - Drag-n-drop (Sortable)
 * - Update status via fetch PATCH
 * - Dropdown aman via x-teleport
 * - Edit modal via fetch JSON (TaskController@edit)
 */
function taskBoard() {
  return {
    // ===== State =====
    ...
    initSortable(key) {
      ...
      const self = this
      const makeSortable = () => {
        // Pastikan Sortable sudah tersedia
        ...
        new Sortable(colEl, {
          ...
          handle: '.card', // seluruh card bisa di-drag
    ...
    async proceedConfirmWA(sendWA) {
      ...
      try {
        ...
        // Sukses: biarkan posisi DOM apa adanya (sudah dipindah Sortable)
      } catch (e) {
        alert(e.message || 'Status gagal diupdate, mengembalikan posisi.')
        // Kembalikan DOM card ke kolom sebelumnya bila perlu (reload paling aman)
        location.reload()
      }
    },
    ...
    // ===== Dropdown Mengambang (x-teleport) =====
    askMove(id) {
      // Placeholder jika mau munculkan dialog custom move
    },
    moveTo(id, to, askWA) {
      if (askWA) {
        this.confirmWA.open   = true
    ...
    async proceedMoveTo(id, to, sendWA) {
      try {
        ...
        // Pindahkan DOM card secara manual (fallback non-drag)
    ...
    async openEdit(id) {
      try {
        ...
        this.edit.form = {
          ...
          due_date: t.due_date || '',
          link: t.link || '',
          owner_ids: Array.from(t.assignee_ids || []),
          notify_wa: false
        }
...
/**
 * Komponen dropdown mengambang (anti tertutup container overflow)
 */
function floatMenu() {
  return {
    open: false,
