                  ...
                  <div class="card-actions justify-between items-center mt-3">
                    ...
                    <div class="flex items-center gap-1">
                      ...
                      <details class="dropdown dropdown-end">
                        ...
                        <ul class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-48 z-10 border border-base-300/50">
                          <li><button type="button" onclick="openEditModal({{ $task->id }})">Edit Tugas</button></li>
                          <div class="divider my-1"></div>
                          <li><button type="button" onclick="uiMoveStatus({{ $task->id }}, 'open')">Pindah ke To Do</button></li>
                          <li><button type="button" onclick="uiMoveStatus({{ $task->id }}, 'in_progress')">Pindah ke In Progress</button></li>
                          <li><button type="button" onclick="uiMoveStatus({{ $task->id }}, 'review')">Pindah ke Review</button></li>
                          <li><button type="button" onclick="uiMoveStatus({{ $task->id }}, 'done')">Pindah ke Done</button></li>
                          <div class="divider my-1"></div>
                          <li>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.kanban-column').forEach(column => {
    new Sortable(column, {
      ...
      onEnd: async function(evt) {
        ...
        // Optimistic move sudah terjadi via Sortable; tinggal simpan
        await updateTaskStatus(taskId, newStatus, doWa ? 1 : 0);
  ...
  async function updateTaskStatus(taskId, status, notify_wa = 0) {
    try {
  ...
  window.openEditModal = async function(taskId) {
    try {
      ...
      // Preselect multi owners (owner_ids bisa array atau string JSON)
      const ownerSel = document.getElementById('edit_owner_ids');
      if (ownerSel) {
        let ids = task.owner_ids || [];
        if (typeof ids === 'string') { try { ids = JSON.parse(ids); } catch(_) { ids = []; } }
        if (!Array.isArray(ids)) ids = [];
        const idset = new Set(ids.map(v => parseInt(v)));
        [...ownerSel.options].forEach(opt => { opt.selected = idset.has(parseInt(opt.value)); });
      }
