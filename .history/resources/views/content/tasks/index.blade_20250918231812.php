...
@push('styles')
<style>
  /* Kolom kanban tidak memotong dropdown yang di-teleport */
  .kanban-column { overflow: visible; isolation: isolate; }
  /* Menu mengambang di-body */
...
<div x-data="taskBoard()" class="container mx-auto py-6">
  ...
  <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3" data-aos="fade-down">
    <div>
      ...
      <p class="text-base-content/70 mt-1">Kelola tugas secara visual di papan kanban.</p>
    </div>
    <div class="flex items-center gap-2">
      <label class="input input-bordered flex items-center gap-2">
        ...
        <input x-model="filters.q" @input.debounce.250ms="applyFilters" type="text" class="grow" placeholder="Cari judul/PIC..." />
      </label>
  ...
  <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6 mt-6">
    ...
    <div class="kanban-column card bg-base-100 border border-base-300/50 shadow-lg" data-aos="fade-up">
      <div class="card-body p-3">
        ...
        <div class="space-y-3 min-h-[120px]" :id="ids.col('{{ $col['key'] }}')" x-init="$nextTick(() => initSortable('{{ $col['key'] }}'))">
         @forelse ($tasks[$col['key']] as $t)
  @php
    $owners = $t->owners->map->only(['id', 'name', 'wa_number'])->values()->all();
    $payload = [
      'id'       => $t->id,
      'title'    => $t->title,
      'priority' => $t->priority,
  ...
  <div 
    ...
    {{-- gunakan json_encode agar aman untuk atribut single-quoted --}}
    data-task='{{ json_encode($payload, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_AMP|JSON_UNESCAPED_UNICODE) }}'
  >
    <div class="card-body p-3">
      <div class="flex items-start justify-between gap-2">
        ...
        <div class="relative" x-data="floatMenu()">
          <button type="button" class="btn btn-ghost btn-xs" @click="toggle($el)">opsi</button>
  ...
  <div id="confirm_wa_modal" class="modal modal-bottom sm:modal-middle" x-cloak>
    <div class="modal-box">
      ...
      <p class="py-2 text-sm opacity-80">
        Tugas dipindahkan ke kolom <b x-text="confirmWA.toLabel"></b>. Kirim WA ke semua PIC/Owner?
      </p>
...
@push('scripts')
{{-- Fallback CDN untuk Sortable jika belum ter-bundle via Vite --}}
<script>
...
<script>
/**
 * Alpine component utama: stabil & minimal dependency
 * - Drag-n-drop (Sortable)
 * - Update status via PATCH (notify_wa opsional)
 * - Dropdown mengambang via x-teleport
 * - Edit modal load via JSON TaskController@edit
 */
function taskBoard() {
  return {
    /* ======= STATE ======= */
    ...
    async proceedConfirmWA(sendWA) {
      ...
      try {
        ...
        // Sukses: Sortable sudah memindahkan DOM card.
      } catch (e) {
        alert(e.message || 'Status gagal diupdate, me-refresh halaman.')
        location.reload()
      }
    },
    readTaskData(cardEl) {
    ...
    async proceedMoveTo(id, to, sendWA) {
      try {
        ...
        // Fallback: pindahkan DOM manual jika bukan dari drag
    ...
    async openEdit(id) {
      try {
        ...
        this.edit.form = {
          ...
          due_date: t.due_date || '',
          link: t.link || '',
          // Controller sekarang mengembalikan owner_ids (bukan assignee_ids)
...
/** Dropdown mengambang aman (x-teleport ke body) */
function floatMenu() {
  return {
    open: false,
    style: '',
