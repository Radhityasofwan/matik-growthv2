          ...
          <div id="{{ $status }}" class="kanban-column board-col-body space-y-3 sm:space-y-4" data-status="{{ $status }}">
            @foreach ($tasks[$status] as $task)
              ...
              <ul class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-48 z-10 border border-base-300/50">
                <li>
                  <button type="button" onclick="openEditModal({{ $task->id }})">Edit Tugas</button>
                </li>
...
<dialog id="create_task_modal" class="modal">
  <div class="modal-box w-11/12 max-w-2xl">
    ...
    <form action="{{ route('tasks.store') }}" method="POST" class="mt-4 space-y-4" id="create_form">
      ...
      <select name="owner_ids[]" class="select select-bordered w-full" multiple size="4">
        @foreach($users as $user)
  ...
  <div class="modal-box w-11/12 max-w-2xl">
    ...
    <form id="edit_task_form" action="" method="POST" class="mt-4 space-y-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div class="form-control">
          <label class="label"><span class="label-text">PIC/Owner (boleh lebih dari satu)</span></label>
      <select id="edit_owner_ids" name="owner_ids[]" class="select select-bordered w-full" multiple size="4">
        @foreach($users as $user)
          <option value="{{ $user->id }}">{{ $user->name }}</option>
        @endforeach
      </select>
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  window.openEditModal = async function(taskId) {
    try {
      ...
      if (ownerSel) {
        // task.owner_ids bisa array atau string JSON; normalisasi jadi array of ids
        let ids = task.owner_ids || [];
        if (typeof ids === 'string') { try { ids = JSON.parse(ids); } catch(_) { ids = []; } }
        if (!Array.isArray(ids)) ids = [];
        [...ownerSel.options].forEach(opt => { opt.selected = ids.includes(parseInt(opt.value)); });
      }
