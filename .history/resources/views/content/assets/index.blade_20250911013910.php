    ...
    @foreach ($files as $f)
      <div class="card bg-base-100 shadow-xl">
        <figure class="bg-base-200 aspect-video">
          @if($f->is_image)
            {{-- Preview thumbnail tetap dari route preview agar konsisten; gunakan ?path= --}}
            <img src="{{ route('assets.preview', ['path' => $f->path]) }}" alt="{{ $f->name }}" class="object-contain max-h-48 p-2">
          @else
        ...
        <div class="card-body">
          ...
          <div class="card-actions justify-end">
            <a href="{{ route('assets.preview', ['path' => $f->path]) }}" target="_blank" class="btn btn-outline btn-sm">Preview</a>
