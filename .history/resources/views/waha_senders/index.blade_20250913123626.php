            ...
            <div class="inline-block min-w-full shadow-md rounded-lg overflow-hidden">
                <table class="min-w-full leading-normal">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Default</th>
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Nama</th>
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Nomor</th>
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Session</th>
                            <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold uppercase">Aktif</th>
                    ...
                    <tbody class="bg-white dark:bg-gray-800">
                        ...
                        <tr>
                            ...
                            <td class="px-5 py-5 border-b text-sm">{{ $s->number }}</td>
                            <td class="px-5 py-5 border-b text-xs font-mono break-all">{{ $s->session }}</td>
                            <td class="px-5 py-5 border-b text-sm">
...
{{-- Inline script agar pasti dieksekusi (tanpa @push) --}}
<script>
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('form.ajax-post').forEach(function (form) {
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            const btn = form.querySelector('button[type="submit"]');
            if (btn) { btn.disabled = true; btn.classList.add('loading'); }
            fetch(form.action, { method:'POST', headers:{ 'X-Requested-With':'XMLHttpRequest','X-CSRF-TOKEN':'{{ csrf_token() }}','Accept':'application/json' }})
            .then(r => r.ok ? r.json() : r.json().then(j => Promise.reject(j)))
            .then(() => location.reload())
            .catch(err => { alert(err?.message || 'Aksi gagal diproses.'); if (btn){ btn.disabled=false; btn.classList.remove('loading'); } });
        });
    });
});
</script>
