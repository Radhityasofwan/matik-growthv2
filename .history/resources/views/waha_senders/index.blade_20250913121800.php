...
<script>
function senderPage() {
    // === ROUTES: pakai named route sesuai web.php ===
    const ROUTE_INDEX_JSON   = @json(route('waha-senders.index', ['json' => 1]));
    const ROUTE_STORE        = @json(route('waha-senders.store'));
    const ROUTE_UPDATE_TPL   = @json(route('waha-senders.update',   ['wahaSender' => '__ID__']));
    const ROUTE_DESTROY_TPL  = @json(route('waha-senders.destroy',  ['wahaSender' => '__ID__']));
    ...
    // helper ganti placeholder __ID__
    ...
    return {
        ...
        async submit(){
            ...
            try{
                ...
                if(isEdit) fd.set('_method','PUT'); // resource update expects PUT/PATCH
        ...
        // TOGGLE ACTIVE (POST ke waha-senders.{id}.toggle-active)
        ...
        // SET DEFAULT (POST ke waha-senders.{id}.set-default)
        async setDefault(s){
        ...
        // DESTROY (DELETE ke waha-senders.{id})
