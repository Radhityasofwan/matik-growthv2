...
<script>
  ...
  .then(async (res) => {
    ...
    if (!res.ok || js.success === false) {
      ...
      // ⚠️ tampilkan sementara, tapi ini tidak blocking
      show('scan-error-' + id);
      el('scan-error-' + id).textContent = `Gagal memanggil endpoint QR: ${msg}`;
    }
  })
  .catch(() => {
    show('scan-error-' + id);
    el('scan-error-' + id).textContent = 'Tidak bisa menghubungi server untuk memulai sesi.';
  })
  .finally(() => {
    // 2) Polling status/QR (1 detik sekali)
      ...
      fetch(`{{ url('waha-senders') }}/${id}/qr-status`, { headers: { 'Accept': 'application/json' } })
        .then(async (res) => {
          ...
          if (!res.ok || js.success === false) {
            const msg = js.error || js.message || `HTTP ${res.status}`;
            show('scan-error-' + id);
            el('scan-error-' + id).textContent = `Gagal mengambil status: ${msg}`;
            return;
          }

          // ✅ status sukses → hilangkan banner error (mis. /start 504)
          hide('scan-error-' + id);
        ...
        .catch(() => {
          hide('scan-loading-' + id);
          show('scan-error-' + id);
          el('scan-error-' + id).textContent = 'Gagal mengambil status (network).';
        });
    };

    tick();
    polls[id] = setInterval(tick, 1000); // ⏱ lebih cepat supaya tangkap QR
  });
}
            ...
            if (js.qr) {
            let src = js.qr;
            // kalau backend kirim base64 polos (tanpa data-uri), tambahkan prefix
            if (!/^data:|^https?:\/\//.test(src) && /^[A-Za-z0-9+/=]{200,}$/.test(src)) {
                src = 'data:image/png;base64,' + src;
            }
            const img = el('scan-qr-' + id);
