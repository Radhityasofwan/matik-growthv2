...
<div class="container mx-auto py-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" data-aos="fade-down">
        ...
        {{-- FIXED: Menggunakan onclick untuk membuka modal secara programatik --}}
                        ...
                        <tr class="hover">
                            ...
                            <td class="text-right">
                                <div class="flex gap-2 justify-end">
                                    {{-- FIXED: Menggunakan button dengan onclick terpisah --}}
                                    ...
                                    <div class="dropdown dropdown-end">
                                        ...
                                        <ul tabindex="0" class="dropdown-content menu p-2 shadow bg-base-100 rounded-box w-52 z-10 border border-base-300/50">
                                            ...
                                            <li>
                                                <form action="{{ route('waha-senders.set-default', $s) }}" method="POST" class="w-full">
                                                    @csrf
                                                    <button type="submit" class="w-full text-left p-2 hover:bg-base-200 rounded-lg" @disabled($s->is_default)>Jadikan Default</button>
                                                </form>
                                            </li>
                                            <div class="divider my-1"></div>
                                            <li>
                                                <form action="{{ route('waha-senders.destroy', $s) }}" method="POST" class="w-full" onsubmit="return confirm('Hapus sender ini?')">
                                                    @csrf @method('DELETE')
                                                    <button type="submit" class="w-full text-left text-error p-2 hover:bg-error/10 rounded-lg">Hapus</button>
                                                </form>
                                            </li>
                                        </ul>
                                    </div>
...
{{-- Modals --}}
{{-- Create Modal --}}
{{-- FIXED: Menggunakan <dialog> sebagai elemen modal --}}
...
@foreach ($senders as $s)
    ...
    <dialog id="scan_sender_{{ $s->id }}" class="modal">
        <div class="modal-box max-w-2xl relative overflow-hidden">
            ...
            <div class="mt-4">
                <button type="button" class="btn btn-outline btn-sm" onclick="requestCode({{ $s->id }})">Minta Kode Pairing</button>
            </div>
            ...
            <div id="scan-success-{{ $s->id }}" class="hidden text-center py-8">
                <div class="mx-auto w-20 h-20 rounded-full bg-success/10 flex items-center justify-center">
                    <svg class="w-12 h-12 text-success" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"/><polyline points="22 4 12 14.01 9 11.01"/></svg>
                </div>
...
@push('scripts')
{{-- Vanilla JS untuk fungsionalitas WAHA --}}
<script>
  ...
  // FIXED: Fungsi baru untuk membuka modal dan memulai scan
  function openAndStartScan(id, name) {
    const modal = document.getElementById('scan_sender_' + id);
    if (modal && typeof modal.showModal === 'function') {
        modal.showModal();
        startScan(id, name);
    } else {
        console.error('Modal for sender ' + id + ' not found or is not a dialog element.');
    }
  }
