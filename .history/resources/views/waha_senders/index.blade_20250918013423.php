{{-- resources/views/waha-senders/index.blade.php --}}
@extends('layouts.app')
...
<div class="container mx-auto py-6">
    ...
    @if ($errors->any())
        <div class="alert alert-error shadow-lg mb-6" data-aos="fade-down">
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" class="stroke-current flex-shrink-0 h-6 w-6" fill="none" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 14l2-2m0 0l2-2m-2 2l-2-2m7-2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
    ...
    @else
        {{-- Table --}}
        <div class="card bg-base-100 shadow-lg border border-base-300/50 mt-8 !overflow-visible" data-aos="fade-up">
                ...
                <table class="table w-full min-w-max">
                    ...
                    @forelse ($senders as $s)
                        <tr class="hover">
                            ...
                            {{-- Aksi: tombol + flyout kiri (anti-terpotong) --}}
                            <td class="text-right">
                                <div class="flex gap-2 justify-end items-center">
                                    <button type="button" class="btn btn-primary btn-sm"
                                            onclick="openAndStartScan({{ $s->id }}, @js($s->name))">
                                        Scan / Connect
                                    </button>
                                    ...
                                    <div class="relative" x-data="{ open:false }">
                                        ...
                                        <div x-cloak x-show="open"
                                             ...
                                             class="absolute right-full mr-2 top-1/2 -translate-y-1/2 z-50 w-64 max-w-[calc(100vw-2rem)]">
                                            <div class="card bg-base-100 text-base-content border border-base-300/60 shadow-2xl rounded-xl">
                                                <ul class="menu menu-sm p-2 max-h-[70vh] overflow-auto">
                                                    <li>
                                                        <button type="button" @click="open=false; document.getElementById('edit_sender_{{ $s->id }}').showModal()">
                                                            <svg class="h-4 w-4 opacity-70" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04a1.003 1.003 0 0 0 0-1.42l-2.34-2.34a1.003 1.003 0 0 0-1.42 0l-1.83 1.83 3.75 3.75 1.84-1.82z"/></svg>
                                                            Edit
                                                        </button>
                                                    </li>
                                                    <li>
                                                        <a href="#" @click.prevent="open=false; document.getElementById('form-default-{{ $s->id }}').submit()"
                                                           class="{{ $s->is_default ? 'pointer-events-none opacity-50' : '' }}">
                                                            <svg class="h-4 w-4 opacity-70" viewBox="0 0 24 24" fill="currentColor"><path d="M12 3v12m0 0l-4-4m4 4l4-4M4 21h16"></path></svg>
                                                            Jadikan Default
                                                        </a>
                                                    </li>
                                                    ...
                                                    <li>
                                                        <a href="#"
                                                           @click.prevent="open=false; if(confirm('Hapus sender ini?')) document.getElementById('form-delete-{{ $s->id }}').submit()"
                                                           class="text-error">
                                                            <svg class="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M6 7h12m-9 3v8m6-8v8M9 4h6l1 2H8l1-2z"/></svg>
                                                            Hapus
                                                        </a>
                                                    </li>
                                                </ul>
