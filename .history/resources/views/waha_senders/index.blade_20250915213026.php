...
<script>
  ...
  function startScan(id, name){
    ...
    // FIRE-AND-FORGET: mulai sesi (tidak menunggu)
    ...
    const tick = () => {
      ...
      fetch(`{{ url('waha-senders') }}/${id}/qr-status`, { headers: { 'Accept': 'application/json' } })
        .then(async (res) => {
          ...
          // status berhasil → bersihkan error sementara
          pollStarted[id] = true;
          clearErr(id);
          ...
          if (state === 'SCAN_QR_CODE') {
            ...
            // 2) fallback: URL proxy (Laravel stream → WAHA /api/{session}/auth/qr)
            else if (js.qr_url) {
              const img = $('scan-qr-' + id);
              if (img) {
                const loadQr = () => { img.src = js.qr_url + '?t=' + Date.now(); };
                img.onerror = () => setTimeout(loadQr, 1200); // retry saat 404/504
                loadQr();
              }
              show('scan-qr-wrap-' + id);
            } else {
            ...
            // bukan scan mode → sembunyikan QR
            hide('scan-qr-wrap-' + id);
          }

          // === selesai jika sudah connected ===
