...
<script>
function senderPage() {
    const API_LIST = @json(route('waha-senders.index', ['json' => 1]));
    const ROUTE_STORE = @json(route('waha-senders.store'));
    const URL_BASE = @json(url('waha-senders'));
    ...
    return {
        ...
        get filtered() {
            ...
            return this.rows.filter(s =>
                ...
                (s.number||'').toLowerCase().includes(key) ||
                (s.session||'').toLowerCase().includes(key)
            );
        },
        ...
        async load() {
            ...
            try {
                const r = await fetch(API_LIST, { headers: { 'Accept':'application/json' }});
                ...
                // sort sama dengan controller: default desc, lalu name jika ada
                this.rows.sort((a,b)=>{
        ...
        async submit(){
            ...
            try {
                ...
                const url = isEdit ? `${URL_BASE}/${this.form.id}` : ROUTE_STORE;
                ...
                if (!r.ok) {
                    ...
                    if (json?.errors) {
                        ...
                        throw new Error(Array.isArray(first) ? first[0] : (first || 'Gagal menyimpan.'));
                    }
                    throw new Error(json?.message || 'Gagal menyimpan.');
                }

                // refresh dari sumber kebenaran
                await this.load();
                this.showModal = false;
            } catch(e) {
                this.formError = e?.message || 'Gagal menyimpan.';
            } finally {
                this.saving = false;
            }
        },
        ...
        async toggleActive(s){
            this.busyId = s.id;
            try {
                const r = await fetch(@json(route('waha-senders.toggle-active', 0)).replace(/0$/, s.id), {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': CSRF, 'Accept':'application/json' },
                });
                const json = await r.json();
                if (!r.ok) throw new Error(json?.message || 'Gagal memperbarui status.');
                // sinkronkan
                const found = this.rows.find(x=>x.id===s.id);
                if (found) found.is_active = !!json.data.is_active;
            } catch(e) {
        ...
        async setDefault(s){
            ...
            this.busyId = s.id;
            try {
                const r = await fetch(@json(route('waha-senders.set-default', 0)).replace(/0$/, s.id), {
                    method: 'POST',
                    headers: { 'X-CSRF-TOKEN': CSRF, 'Accept':'application/json' },
                });
                const json = await r.json();
                if (!r.ok) throw new Error(json?.message || 'Gagal set default.');

                // pastikan hanya satu default & default selalu aktif (mengikuti controller)
                this.rows.forEach(x => x.is_default = false);
                const found = this.rows.find(x=>x.id===s.id);
                if (found) { found.is_default = true; found.is_active = true; }

                // opsional: re-sort agar default selalu di atas
                this.rows.sort((a,b)=>{
                    if (a.is_default !== b.is_default) return a.is_default ? -1 : 1;
                    return (a.name||'').localeCompare(b.name||'');
                });
            } catch(e) {
                alert(e?.message || 'Gagal set default.');
            } finally { this.busyId = null; }
        },
        ...
        async confirmDelete(s){
            ...
            this.busyId = s.id;
            try {
                const fd = new URLSearchParams({'_method':'DELETE'});
                const r = await fetch(`${URL_BASE}/${s.id}`, {
                    method: 'POST',
