...
<script>
  ...
  // Simpan interval per-sender
  const polls = {};          // { [id]: intervalId }
  const pollStarted = {};    // { [id]: bool }  → sudah pernah dapat status OK
  const pollDeadline = {};   // { [id]: ts }    → putuskan polling setelah X detik
  ...
  function startScan(id, name){
    ...
    // 0) Stop polling lama, set deadline (45s)
    if (polls[id]) clearInterval(polls[id]);
    pollStarted[id] = false;
    pollDeadline[id] = Date.now() + 45_000;

    // 1) FIRE-AND-FORGET: panggil /qr-start tapi TIDAK menunggu hasilnya
    //    (agar UI tidak “ngegantung” saat WAHA lambat/504)
    ...
    .then(async (res) => {
      ...
      // Kalau /start gagal tetapi polling sudah sukses → abaikan error ini
    ...
    .catch(() => {
      // network error start, tapi kita tetap lanjut polling; tampilkan info ringan
      if (!pollStarted[id]) showErrorFor(id, 'Tidak bisa menghubungi server untuk memulai sesi.');
    });

    // 2) MULAI POLLING LANGSUNG (tiap 1s)
    const tick = () => {
      // hentikan jika lewat deadline
      if (Date.now() > (pollDeadline[id] || 0)) {
        if (polls[id]) { clearInterval(polls[id]); polls[id] = null; }
        hide('scan-loading-' + id);
        showErrorFor(id, 'Timeout menunggu QR. Coba ulangi.');
        return;
      }
      ...
      fetch(`{{ url('waha-senders') }}/${id}/qr-status`, { headers: { 'Accept': 'application/json' } })
        .then(async (res) => {
          ...
          // Sukses dapat status → bersihkan banner error yang mungkin tersisa dari /start
          pollStarted[id] = true;
          clearErrorFor(id);
          ...
          const state = (js.state ? String(js.state) : 'UNKNOWN').toUpperCase();
            ...
            // 1) jika backend sudah memberikan data-URI/base64
            if (js.qr) {
            let src = js.qr;
            ...
            // 2) fallback: gunakan URL proxy (server-side stream)
            else if (js.qr_url) {
            const img = el('scan-qr-' + id);
            if (img) img.src = js.qr_url + '?t=' + Date.now(); // cache-bust
            show('scan-qr-wrap-' + id);
            } else {
            hide('scan-qr-wrap-' + id);
            }

          // Selesai bila sudah CONNECTED
    ...
    // panggilan pertama + interval 1 detik
    tick();
