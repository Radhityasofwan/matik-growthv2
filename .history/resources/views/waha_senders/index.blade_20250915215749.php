...
<div id="scan-code-wrap-{{ $s->id }}" class="mt-3 hidden">
  <p class="text-sm mb-2">
    Atau tautkan dengan kode berikut di WhatsApp
    (Linked Devices → <i>Link with phone number / Masuk dengan kode</i>).
  </p>
  <div class="flex items-center gap-3">
    <div id="scan-code-{{ $s->id }}" class="text-3xl font-mono tracking-widest">
      ————
    </div>
...
<script>
  ...
  function requestCode(id){
    // tampilkan loading kecil di badge state
    const badge = document.getElementById('scan-state-' + id);
    ...
    .then(js => {
      if (!js || js.success === false) {
        ...
        const err = document.getElementById('scan-error-' + id);
        if (err) { err.textContent = msg; err.classList.remove('hidden'); }
        return;
      }
      // tampilkan kode
      const code = (js.code || '').toString().trim();
      const elCode = document.getElementById('scan-code-' + id);
      if (elCode) elCode.textContent = code || '—';
      const wrap = document.getElementById('scan-code-wrap-' + id);
    ...
    .catch(() => {
      const err = document.getElementById('scan-error-' + id);
      if (err) { err.textContent = 'Tidak dapat meminta kode.'; err.classList.remove('hidden'); }
    });
  }
</script>
@endsection
