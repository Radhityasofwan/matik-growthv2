...
<aside
  ...
  class="peer group/aside fixed inset-y-0 left-0 z-50 flex flex-col bg-base-100 border-r border-base-300/50 transition-all duration-300 ease-in-out
         w-72 -translate-x-full lg:translate-x-0 lg:w-20 lg:hover:w-64 lg:focus-within:w-64"
  :class="{'translate-x-0 shadow-xl': isMobileMenuOpen}"
  x-cloak
>
  {{-- Brand --}}
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      ...
      {{-- Teks selalu tampil di mobile; di desktop disembunyikan saat kolaps --}}
      <span class="whitespace-nowrap text-lg font-bold text-base-content
                   lg:opacity-0 lg:translate-x-2 lg:transition-all lg:duration-200
                   lg:group-hover/aside:opacity-100 lg:group-hover/aside:translate-x-0
                   lg:focus-within:opacity-100 lg:focus-within:translate-x-0">
        MatikGrowth
      </span>
    </a>
  </div>
  ...
  <nav class="flex-1 overflow-y-auto">
    ...
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title px-4 text-base-content/50
                   lg:opacity-0 lg:transition-opacity lg:duration-150
                   lg:group-hover/aside:opacity-100 lg:focus-within:opacity-100">
          <span>{{ $group }}</span>
        </li>
        ...
        @foreach ($links as $link)
          ...
          <li>
            <a href="{{ route($link['route']) }}"
               @click="isMobileMenuOpen=false"
               class="flex items-center gap-3 {{ $isActive ? 'bg-primary text-primary-content' : '' }}"
            >
              {!! $link['icon'] !!}
              <span class="truncate
                           lg:opacity-0 lg:translate-x-2 lg:transition-all lg:duration-150
                           lg:group-hover/aside:opacity-100 lg:group-hover/aside:translate-x-0
                           lg:focus-within:opacity-100 lg:focus-within:translate-x-0">
                {{ $link['label'] }}
              </span>
