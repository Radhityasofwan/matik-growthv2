{{-- Overlay mobile (tap untuk menutup) – tampil hanya saat menu terbuka --}}
<div
...
<aside
  id="app-sidebar"
  data-aos="fade-right"
  class="fixed inset-y-0 left-0 z-50 flex flex-col bg-base-100 border-r border-base-300/50 transition-transform duration-300 ease-in-out
         w-64 lg:translate-x-0"
  :class="{
    'translate-x-0 shadow-xl': isMobileMenuOpen,        // mobile: tampil
    '-translate-x-full': !isMobileMenuOpen,              // mobile: sembunyi
    'lg:w-64': isSidebarExpanded,                        // desktop expanded
    'lg:w-20': !isSidebarExpanded                        // desktop collapsed
  }"
  role="navigation"
  aria-label="Sidebar"
  :aria-hidden="(!isMobileMenuOpen && $screen('lg') === false) ? 'true' : 'false'"
  x-cloak
>
  {{-- Brand --}}
  ...
  <nav class="flex-1 overflow-y-auto">
    ...
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title text-base-content/60 transition-all duration-300"
            :class="isSidebarExpanded ? 'px-4' : 'text-center'">
          ...
          <li>
            <a href="{{ route($link['route']) }}"
               ...
               :class="{
                  ...
                  'tooltip tooltip-right': !isSidebarExpanded
               }"
               data-tip="{{ $link['label'] }}"
               title="{{ $link['label'] }}"
               role="link"
  ...
  <div class="p-4 border-t border-base-300/50">
    <button
      ...
      :aria-expanded="isSidebarExpanded.toString()"
