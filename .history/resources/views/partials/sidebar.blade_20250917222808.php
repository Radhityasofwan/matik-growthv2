...
<aside
  id="app-sidebar"
  class="sidebar-shell group/sidebar fixed inset-y-0 left-0 z-50 flex flex-col bg-base-100 border-r border-base-300/50 transition-all duration-300 ease-in-out will-change-transform"
  :class="{
    // MOBILE drawer behavior
    'w-72 translate-x-0 shadow-2xl mobile-open': isMobileMenuOpen,   // <-- tambahkan 'mobile-open' saat terbuka
    'w-72 -translate-x-full': !isMobileMenuOpen,

    // DESKTOP hover collapse / expand (base collapsed)
...
>
  ...
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4"
    ...
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      ...
      <span
        class="brand-label origin-left whitespace-nowrap text-lg font-bold text-base-content hidden lg:block"
  ...
  @php
    $navGroups = [
      ...
      'Content' => [
        ['route' => 'tasks.index', 'label' => 'Tasks', 'icon' => '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 22h2a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 1-2 2v16"/><path d="M9 12l2 2 4-4"/></svg>'],
  ...
  <nav class="flex-1 overflow-y-auto">
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title">
          <span :class="sidebarHover ? '' : 'lg:opacity-0 lg:pointer-events-none'">{{ $group }}</span>
        </li>
        ...
        @foreach ($links as $link)
          ...
          <li>
            ...
            >
              {!! $link['icon'] !!}
              <span class="label whitespace-nowrap hidden lg:inline" :class="sidebarHover ? 'opacity-100' : 'opacity-0 pointer-events-none'">
                {{ $link['label'] }}
              </span>
            </a>
          </li>
  ...
  <div class="p-4 border-t border-base-300/50 hidden lg:block">
    <div class="footer-note text-xs text-base-content/50" :class="sidebarHover ? '' : 'opacity-0 pointer-events-none'">
      v2
    </div>
  </div>
</aside>
