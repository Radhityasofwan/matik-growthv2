...
<div class="sticky top-0 z-30 p-4 pb-0 sm:p-6 sm:pb-0">
  <header data-aos="fade-down" class="navbar rounded-box border border-base-300/20 bg-base-100/80 shadow-md backdrop-blur-lg" role="banner">
    <div class="navbar-start gap-2">
      ...
      <button
        type="button"
        @click="$store.ui.isMobileMenuOpen = !$store.ui.isMobileMenuOpen"
      ...
      <div class="hidden lg:flex items-center">
        <span class="font-semibold text-base-content/80">
          <span x-text="$store.ui.greet()"></span>, {{ Str::of(Auth::user()->name)->explode(' ')->first() }} 👋
        </span>
      </div>
    </div>
    ...
    <div class="navbar-end gap-1">
      ...
      <button
        type="button"
        @click="$store.ui.isSearchOpen = true; setTimeout(()=>document.querySelector('[data-search-input]')?.focus(),0)"
      ...
      {{-- Theme picker (pakai Alpine Store) --}}
      <div class="dropdown dropdown-end" x-data="{ q: '', filtered(){ const s=this.q.toLowerCase().trim(); return s ? $store.ui.themes.filter(t=>t.toLowerCase().includes(s)) : $store.ui.themes; } }">
        <label tabindex="0" class="btn btn-ghost btn-circle" aria-label="Pilih Tema">
          <div class="indicator">
            ...
            <span class="indicator-item badge badge-xs translate-x-1 translate-y-1" :data-theme="$store.ui.theme"></span>
          </div>
        </label>
          ...
          <div class="p-3">
            ...
            <ul class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-80 overflow-y-auto pr-1">
              <template x-for="t in filtered()" :key="t">
                <li>
                  <button type="button" @click.prevent="$store.ui.setTheme(t)" class="w-full group rounded-xl border border-base-300/60 hover:border-primary/60 transition-colors p-2 text-left" :class="{'ring-2 ring-primary/60': $store.ui.theme===t}">
                    ...
                    <div class="mt-1.5 flex items-center justify-between">
                      ...
                      <span class="badge badge-ghost badge-xs" x-show="$store.ui.theme===t">aktif</span>
                    </div>
                  </button>
                </li>
              </template>
      ...
      {{-- Notifs --}}
      <div class="dropdown dropdown-end" x-data="notifBell($el)" x-init="init()"
