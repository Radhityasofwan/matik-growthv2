...
>
    <!-- Brand/Logo -->
    <div class="flex items-center shrink-0 h-16 border-b border-base-300/50 px-4" :class="isSidebarExpanded ? 'justify-between' : 'justify-center'">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
            {{-- Menggunakan komponen logo Anda --}}
            <x-application-logo class="w-8 h-8 text-primary"/>
            <span
                x-show="isSidebarExpanded"
                x-transition:enter="transition ease-out duration-200"
                x-transition:enter-start="opacity-0"
                class="text-lg font-bold text-neutral whitespace-nowrap"
            >
                MatikGrowth
            </span>
        </a>
    ...
    <nav class="flex-1 overflow-y-auto">
        ...
        <ul class="menu p-4" :class="{'w-full': isSidebarExpanded, 'w-20': !isSidebarExpanded}">
            @foreach ($navGroups as $group => $links)
                <li class="menu-title" :class="{'px-0': isSidebarExpanded, 'text-center': !isSidebarExpanded}">
                    <span x-show="isSidebarExpanded" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0">{{ $group }}</span>
                    <span x-show="!isSidebarExpanded" class="text-xs">...</span>
                </li>
    ...
    <div class="p-2 border-t border-base-300/50">
        ...
        >
            <span x-show="isSidebarExpanded" class="text-sm mr-2">Collapse</span>
