@php
    // Nilai awal dari View Composer (AppServiceProvider)
    $unreadInit = $unreadNotificationsCount ?? 0;
    $previewInit = $notificationsPreview ?? [];
@endphp

<header
    x-data="notifBell('{{ url('/notifications') }}', {{ (int) $unreadInit }}, @json($previewInit))"
    x-init="init()"
...
>
    ...
    <div class="flex items-center gap-4">
        <!-- Notifikasi (dropdown + auto-poll) -->
        <div class="dropdown dropdown-end" @keydown.escape="$dispatch('close-all')">
            <button @click="open = !open; if(open) fetchFeed()" class="btn btn-ghost btn-circle">
                <div class="indicator">
                    ...
                    <template x-if="unread > 0">
                        <span class="badge badge-sm badge-primary indicator-item" x-text="unread"></span>
                    </template>
                </div>
            </button>
            ...
            <div class="dropdown-content z-10 mt-3 w-80 rounded-xl shadow-xl bg-base-100 border border-base-300/50">
                <div class="p-3 border-b border-base-300/50 flex items-center justify-between">
                    ...
                    <button class="btn btn-ghost btn-xs" @click="markAll()">Tandai semua dibaca</button>
                </div>
                            ...
                            >
                                <div class="mt-0.5">
                                    <span class="w-2.5 h-2.5 rounded-full"
                                          :class="n.read_at ? 'bg-base-300' : 'bg-primary'"></span>
                                </div>
                                <div class="min-w-0">
...
@push('scripts')
<script>
function notifBell(baseUrl, unreadInit = 0, preview = []) {
    return {
        open: false,
        unread: unreadInit,
        items: preview || [],
        ...
        feedUrl() { return baseUrl + '/feed'; },
        readUrl(id) { return baseUrl + '/' + encodeURIComponent(id) + '/read'; },
        markAllUrl() { return baseUrl + '/read-all'; },
