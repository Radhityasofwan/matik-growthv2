{{-- resources/views/partials/sidebar.blade.php — FINAL (tanpa app.css) --}}
...
<aside
  ...
  aria-modal="true"
  data-aos="fade-right"
  class="fixed inset-y-0 left-0 z-[70] flex flex-col bg-base-100 border-r border-base-300/50
         transition-all duration-300 ease-out rounded-r-2xl shadow-2xl lg:shadow-none
         backdrop-blur-xl supports-[backdrop-filter]:bg-base-100/90 lg:backdrop-blur-0"
  :class="{
    // MOBILE
    'w-72 translate-x-0': isMobileMenuOpen,
    'w-72 -translate-x-full': !isMobileMenuOpen,
    // DESKTOP (hover expand)
    'lg:translate-x-0 lg:w-64': isSidebarHover,
    'lg:translate-x-0 lg:w-20': !isSidebarHover
  }"
  x-data="{ ts: 0 }"
  @mouseenter="if (window.innerWidth >= 1024) isSidebarHover = true"
  @mouseleave="if (window.innerWidth >= 1024) isSidebarHover = false"
...
>
  {{-- Brand --}}
  <div
    class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4 transition-all"
    :class="isSidebarHover ? 'lg:justify-start' : 'lg:justify-center'"
  >
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3" @click="if (window.innerWidth < 1024) isMobileMenuOpen=false">
      ...
      {{-- Label brand: tampil saat mobile open ATAU desktop hover --}}
      <span
        x-show="isMobileMenuOpen || isSidebarHover"
        x-cloak
  ...
  <nav class="flex-1 overflow-y-auto">
    ...
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        {{-- Group title: tampil saat mobile open / desktop hover --}}
        <li class="menu-title px-4" x-show="isMobileMenuOpen || isSidebarHover" x-cloak>
          <span>{{ $group }}</span>
        </li>
        ...
        @foreach ($links as $link)
          ...
          <li>
            <a
              ...
              class="flex items-center h-11 rounded-xl transition-all duration-200 ease-out
                     gap-0 justify-center px-3"
              :class="{
                'bg-primary text-primary-content': {{ $isActive ? 'true' : 'false' }},
                // Mobile open → tampilan expanded
                'justify-start gap-3 px-4': isMobileMenuOpen,
                // Desktop hover → tampilan expanded
                'lg:justify-start lg:gap-3 lg:px-4': isSidebarHover,
                // Desktop collapsed → ikon center
                'lg:justify-center lg:gap-0 lg:px-3': !isSidebarHover
              }"
            >
              {!! $link['icon'] !!}
              <span
  ...
  <div class="p-4 border-t border-base-300/50">
    <div class="text-xs text-base-content/60" x-show="isMobileMenuOpen || isSidebarHover" x-cloak>
      MatikGrowth © {{ date('Y') }}
    </div>
  </div>
</aside>
