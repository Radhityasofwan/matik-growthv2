...
<aside
  ...
  :class="{
    ...
    // DESKTOP: selalu terlihat, width mengikuti hover
    'lg:translate-x-0': true,
    'lg:w-64': isSidebarHover,
    'lg:w-20': !isSidebarHover
  }"
  x-cloak

  {{-- Hover handler hanya aktif di desktop --}}
  @mouseenter="if (isDesktop()) isSidebarHover = true"
  @mouseleave="if (isDesktop()) isSidebarHover = false"
>
  {{-- Brand --}}
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4"
       :class="isSidebarHover ? 'justify-start' : 'justify-center'">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      <x-application-logo class="h-8 w-8 shrink-0 text-primary"/>
      <span
        x-show="isSidebarHover"
  ...
  <nav class="flex-1 overflow-y-auto">
    ...
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title transition-all duration-300"
            :class="isSidebarHover ? 'px-4' : 'text-center'">
          <span x-show="isSidebarHover" x-transition>{{ $group }}</span>
          <span x-show="!isSidebarHover" x-transition aria-hidden="true">·</span>
        </li>

        @foreach ($links as $link)
          @php $isActive = request()->routeIs($link['route'] . '*'); @endphp
          <li>
            {{-- Active state mengikuti warna tema DaisyUI --}}
            <a href="{{ route($link['route']) }}"
               class="{{ $isActive ? 'bg-primary text-primary-content' : '' }}"
               :class="{
                  'justify-center': !isSidebarHover,
                  'tooltip tooltip-right': !isSidebarHover
               }"
               data-tip="{{ $link['label'] }}"
            >
              {!! $link['icon'] !!}
  ...
  {{-- Tidak ada tombol Collapse di desktop: sekarang expand via hover --}}
</aside>
