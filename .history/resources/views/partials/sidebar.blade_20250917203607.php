...
<aside
  ...
  data-aos="fade-right"
  class="fixed inset-y-0 left-0 top-0 z-[70] flex flex-col bg-base-100 border-r border-base-300/50
  ...
  :class="{
    ...
    'lg:w-64 lg:translate-x-0': isSidebarHover,
    'lg:w-20 lg:translate-x-0': !isSidebarHover
  }"
  x-data="{ sx: 0 }"
  ...
  @touchend.passive="(() => { const dx = ($event.changedTouches[0]?.clientX || sx) - sx; if (dx < -30) isMobileMenuOpen = false })()"
  x-cloak
>
  {{-- Brand --}}
  <div
  ...
  <nav class="flex-1 overflow-y-auto">
    ...
    <ul class="menu p-4 space-y-1">
      {{-- Group title hanya saat expanded (mobile open / desktop hover) --}}
          ...
          <li>
            <a
              ...
              :class="[
                ...
                (isMobileMenuOpen || isSidebarHover) ? 'justify-start gap-3 px-4' : 'justify-center gap-0 px-3'
              ]"
            >
              {!! $link['icon'] !!}
              <span x-show="isMobileMenuOpen || isSidebarHover" x-cloak class="whitespace-nowrap">{{ $link['label'] }}</span>
            </a>
          </li>
        @endforeach
      @endforeach
