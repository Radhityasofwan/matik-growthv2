...
{{-- ========== MOBILE OVERLAY / DRAWER (HANYA MOBILE) ========== --}}
<div
...
<aside
  ...
  role="dialog"
  aria-modal="true"
  class="fixed inset-y-0 left-0 z-50 lg:hidden sidebar-shell group/sidebar
         w-72 transform transition-transform duration-300 ease-in-out
         bg-base-100 border-r border-base-300/50 will-change-transform"
  :class="$store.ui.isMobileMenuOpen ? 'translate-x-0 shadow-2xl' : '-translate-x-full'"
  x-cloak
>
  {{-- BRAND (selalu expanded di mobile) --}}
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4 justify-start">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      <x-application-logo class="h-8 w-8 shrink-0 text-primary"/>
      <span class="brand-label whitespace-nowrap text-lg font-bold text-base-content">MatikGrowth</span>
    </a>
  </div>

  {{-- MENU (expanded styling via .mobile-open class) --}}
  @php
    $navGroups = [
      'Main' => [
      ...
      @foreach ($navGroups as $group => $links)
        ...
        @foreach ($links as $link)
          ...
          <li>
            ...
            >
              {!! $link['icon'] !!}
              <span class="label whitespace-nowrap">{{ $link['label'] }}</span>
            </a>
          </li>
        @endforeach
      @endforeach
  ...
  <div class="p-4 border-top border-base-300/50">
    <div class="text-xs text-base-content/50">v2</div>
  </div>
</aside>

{{-- ========== DESKTOP SIDEBAR (SELALU ADA, TANPA TRANSLATE) ========== --}}
<aside
  id="app-sidebar"
  class="hidden lg:flex fixed inset-y-0 left-0 z-40 flex-col bg-base-100 border-r border-base-300/50
         transition-[width] duration-300 ease-in-out will-change-[width]
         w-20 hover:w-64 sidebar-shell group/sidebar"
...
>
  ...
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4"
       :class="$store.ui.sidebarHover ? 'justify-start' : 'justify-center'">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      <x-application-logo class="h-8 w-8 shrink-0 text-primary"/>
      <span
        class="brand-label origin-left whitespace-nowrap text-lg font-bold text-base-content"
        :class="$store.ui.sidebarHover ? 'block opacity-100 scale-100' : 'hidden opacity-0 scale-95 pointer-events-none'"
      >
        MatikGrowth
      </span>
    </a>
  ...
  <nav class="flex-1 overflow-y-auto">
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title">
          <span :class="$store.ui.sidebarHover ? '' : 'lg:opacity-0 lg:pointer-events-none'">{{ $group }}</span>
        </li>
        ...
        @foreach ($links as $link)
          ...
          <li>
            <a
              ...
              @click="$dispatch('sidebar:navigated')"
              class="gap-3 {{ $isActive ? 'bg-primary text-primary-content' : '' }}"
              :class="$store.ui.sidebarHover ? '' : 'lg:justify-center'"
            >
              {!! $link['icon'] !!}
              <span
                class="label whitespace-nowrap hidden lg:inline"
                :class="$store.ui.sidebarHover ? 'opacity-100' : 'opacity-0 pointer-events-none'"
              >
                {{ $link['label'] }}
              </span>
            </a>
  ...
  <div class="p-4 border-t border-base-300/50 hidden lg:block">
    <div class="text-xs text-base-content/50" :class="$store.ui.sidebarHover ? '' : 'opacity-0 pointer-events-none'">
      v2
    </div>
  </div>
</aside>
