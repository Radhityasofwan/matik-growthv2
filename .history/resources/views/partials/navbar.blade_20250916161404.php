...
function notifBell(baseUrl, unreadInit = 0, preview = []) {
    return {
        open: false,
        unread: unreadInit,
        items: (preview || []).map(p => ({ source: p.source ?? 'notification', ...p })), // normalisasi awal
        pollMs: 10000,
        timer: null,
        ...
        async markReadIfNotification(n) {
            if (n.source !== 'notification') return; // fallback activity: tidak ada mark-read
            try {
        ...
        async markAll() {
            // hanya jalan kalau ada notifikasi database yang belum dibaca
