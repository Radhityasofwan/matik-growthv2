...
{{-- Overlay (mobile) --}}
<div x-show="isMobileMenuOpen" x-transition.opacity @click="isMobileMenuOpen = false" class="fixed inset-0 z-40 bg-black/50 backdrop-blur-sm lg:hidden" aria-hidden="true" x-cloak></div>

{{-- Sidebar --}}
<aside
  id="app-sidebar"
  data-aos="fade-right"
  class="peer group/sidebar fixed inset-y-0 left-0 z-50 flex flex-col bg-base-100 border-r border-base-300/50
         transition-all duration-300 ease-out lg:translate-x-0
         rounded-r-2xl lg:rounded-none shadow-2xl lg:shadow-none
         backdrop-blur-xl supports-[backdrop-filter]:bg-base-100/90 lg:backdrop-blur-0"
  :class="{
    'w-72 translate-x-0': isMobileMenuOpen,          // mobile open
    'w-72 -translate-x-full': !isMobileMenuOpen      // mobile closed (default)
  }"
  x-cloak
>
  {{-- Brand --}}
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4 lg:justify-center group-hover/sidebar:lg:justify-start transition-all">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      <x-application-logo class="h-8 w-8 shrink-0 text-primary"/>
      <span class="origin-left whitespace-nowrap text-lg font-bold text-base-content hidden lg:group-hover/sidebar:inline-block">MatikGrowth</span>
    </a>
  </div>
    ...
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title px-0">
          <span class="hidden lg:group-hover/sidebar:block px-4">{{ $group }}</span>
          <span class="text-center lg:block lg:group-hover/sidebar:hidden" aria-hidden="true">·</span>
        </li>
        ...
        @foreach ($links as $link)
          ...
          <li>
            <a href="{{ route($link['route']) }}"
               class="gap-3 {{ $isActive ? 'bg-primary text-primary-content' : '' }} tooltip tooltip-right"
               data-tip="{{ $link['label'] }}"
            >
              {!! $link['icon'] !!}
              <span class="whitespace-nowrap hidden lg:group-hover/sidebar:inline-block">{{ $link['label'] }}</span>
            </a>
          </li>
        @endforeach
      @endforeach
  ...
  <div class="p-4 border-t border-base-300/50">
    <div class="hidden lg:group-hover/sidebar:flex text-xs text-base-content/60">
