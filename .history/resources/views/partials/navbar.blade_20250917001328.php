...
<div class="sticky top-0 z-30 p-4 sm:p-6 pb-0 sm:pb-0">
    <header
        x-data="notifBell($el)"
        x-init="init()"
        data-unread="{{ $unreadInit }}"
        data-preview='@js($previewInit)'
        class="navbar bg-base-100/70 backdrop-blur-lg rounded-box shadow-sm border border-base-300/20"
    >
        {{-- Bagian Kiri: Tombol Hamburger & Search --}}
        <div class="navbar-start">
            {{-- Tombol Hamburger untuk membuka sidebar di mobile, terhubung ke state Alpine di body --}}
            <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="btn btn-ghost btn-circle lg:hidden" aria-label="Buka Menu">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"></path></svg>
            </button>

            {{-- Kolom Pencarian (hanya tampil di layar medium ke atas) --}}
        ...
        <div class="navbar-end">
            {{-- Tombol Theme Toggle (Sinkronisasi) --}}
            <button
                x-data="{
                    theme: localStorage.getItem('theme') || 'softblue',
                    toggle() {
                        this.theme = this.theme === 'softblue' ? 'dark' : 'softblue';
                        localStorage.setItem('theme', this.theme);
                        document.documentElement.setAttribute('data-theme', this.theme);
                    },
                    init() {
                        const currentTheme = localStorage.getItem('theme') || 'softblue';
            ...
            >
                <svg x-show="theme === 'dark'" xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"/></svg>
                ...
                <label tabindex="0" class="btn btn-ghost btn-circle" @click="open = !open; if(open) fetchFeed()" aria-label="Notifikasi">
                    <div class="indicator">
                        ...
                        <template x-if="unread > 0">
                            <span class="badge badge-sm badge-primary indicator-item" x-text="unread > 9 ? '9+' : unread" aria-live="polite"></span>
                        </template>
                    </div>
                </label>
                <div tabindex="0" class="dropdown-content z-[1] mt-4 w-80 sm:w-96 bg-base-100 shadow-xl rounded-box border border-base-300/50">
                    ...
                    <ul class="max-h-80 overflow-y-auto menu p-2">
                        <template x-if="items.length === 0">
                            <li class="p-4 text-sm text-neutral/60 text-center">Belum ada notifikasi baru.</li>
                        </template>
            ...
            {{-- Dropdown Profil --}}
            <div class="dropdown dropdown-end">
                <label tabindex="0" class="btn btn-ghost flex items-center gap-2 ml-2">
                    <div class="avatar">
                ...
                <ul tabindex="0" class="menu menu-sm dropdown-content mt-4 z-[1] p-2 shadow bg-base-100 rounded-box w-52 border border-base-300/50">
                    <li class="p-2">
