...
>
  {{-- Brand --}}
  <div class="flex h-20 items-center gap-3 border-b border-base-300/50 px-4">
    <x-application-logo class="h-8 w-8 text-primary" />
    <span class="text-lg font-bold text-base-content">MatikGrowth</span>
  </div>

  {{-- Menu (mobile selalu expanded dengan teks) --}}
  @php
    $navGroups = [
      'Main' => [
...
<aside
  class="group/sidebar fixed inset-y-0 left-0 z-30 hidden lg:flex lg:flex-col lg:bg-base-100 lg:border-r lg:border-base-300/50 lg:transition-[width] lg:duration-300 lg:ease-in-out"
  :class="sidebarHover ? 'lg:w-64' : 'lg:w-20'"
  @mouseenter="sidebarHover = true"
  @mouseleave="sidebarHover = false"
>
  {{-- Brand --}}
  <div class="flex h-20 items-center border-b border-base-300/50 px-4"
       :class="sidebarHover ? 'justify-start' : 'justify-center'">
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
      <x-application-logo class="h-8 w-8 shrink-0 text-primary"/>
      <span class="origin-left text-lg font-bold text-base-content"
            :class="sidebarHover ? 'opacity-100' : 'opacity-0 pointer-events-none'">
        MatikGrowth
      </span>
    </a>
  </div>
  ...
  <nav class="flex-1 overflow-y-auto">
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        <li class="menu-title">
          <span :class="sidebarHover ? '' : 'opacity-0 pointer-events-none'">{{ $group }}</span>
        </li>
        @foreach ($links as $link)
          @php $isActive = request()->routeIs($link['route'] . '*'); @endphp
          <li>
  ...
  <div class="p-4 border-t border-base-300/50">
    <div class="text-xs text-base-content/50" :class="sidebarHover ? '' : 'opacity-0 pointer-events-none'">v2</div>
  </div>
</aside>
