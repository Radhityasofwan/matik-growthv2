...
<div class="sticky top-0 z-30 p-4 pb-0 sm:p-6 sm:pb-0">
  ...
  >
    ...
    {{-- Kanan: Search, Quick Action, Theme Switcher, Notifikasi, Profil --}}
    <div class="navbar-end gap-1">
      ...
      <div
        ...
        x-data="{ q: '', filtered() { const s = this.q.toLowerCase().trim(); return s ? themes.filter(t => t.toLowerCase().includes(s)) : themes; } }"
      >
          ...
          <div class="p-3">
            ...
            <ul class="mt-3 grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-80 overflow-y-auto pr-1">
              <template x-for="t in filtered()" :key="t">
                <li><button type="button" @click.prevent="setTheme(t)" class="w-full group rounded-xl border border-base-300/60 hover:border-primary/60 transition-colors p-2 text-left" :class="{'ring-2 ring-primary/60': theme === t}">
                    <div :data-theme="t" class="rounded-lg p-2 bg-base-200">
                      <div class="flex items-center gap-1.5"><span class="inline-block h-3 w-3 rounded-full bg-primary"></span><span class="inline-block h-3 w-3 rounded-full bg-secondary"></span><span class="inline-block h-3 w-3 rounded-full bg-accent"></span></div><div class="mt-1 h-3 rounded bg-base-300"></div>
                    </div>
                    <div class="mt-1.5 flex items-center justify-between"><span class="text-xs font-medium text-base-content/80" x-text="t.charAt(0).toUpperCase()+t.slice(1)"></span><span class="badge badge-ghost badge-xs" x-show="theme === t">aktif</span></div>
                </button></li>
              </template>
            </ul>
          </div>
        ...
        <div tabindex="0" class="dropdown-content z-[1] mt-4 w-80 sm:w-96 bg-base-100 shadow-lg rounded-box border border-base-300/50">
          ...
          <ul class="max-h-80 overflow-y-auto menu p-2">
            ...
            <template x-for="n in items" :key="n.id">
              <li><a @click.prevent="go(n)" :class="{'opacity-60': n.read_at}">
