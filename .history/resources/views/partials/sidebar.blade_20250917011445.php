...
<aside
    ...
    :class="{
        'translate-x-0 shadow-xl': isMobileMenuOpen,
...
>
    <!-- Logo dan Nama Brand -->
    <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4" :class="isSidebarExpanded ? 'justify-start' : 'justify-center'">
        <a href="{{ route('dashboard') }}" class="flex items-center gap-3">
            ...
            <span
                ...
                class="origin-left whitespace-nowrap text-lg font-bold text-base-content" {{-- UPDATED --}}
            >
                MatikGrowth
            </span>
        </a>
    ...
    <nav class="flex-1 overflow-y-auto">
        @php
            // Struktur menu terpusat untuk kemudahan pengelolaan
            $navGroups = [
                'Main' => [
        ...
        <ul class="menu p-4 space-y-1">
            @foreach ($navGroups as $group => $links)
                <li class="menu-title text-base-content/60 transition-all duration-300" :class="isSidebarExpanded ? 'px-4' : 'text-center'"> {{-- UPDATED --}}
                ...
                @foreach ($links as $link)
                    ...
                    <li>
                        <a href="{{ route($link['route']) }}"
                           ...
                           title="{{ $link['label'] }}"
                           {{-- Tooltip dari DaisyUI untuk mode collapsed --}}
                           x-bind:class="{ 'tooltip tooltip-right': !isSidebarExpanded }"
                           data-tip="{{ $link['label'] }}"
                        >
                            {!! $link['icon'] !!}
                            <span
    ...
    <!-- Tombol Collapse Sidebar (hanya tampil di desktop) -->
