<!--
    ...
    - Menggunakan class transform `translate-x` untuk animasi di mobile.
    - `lg:translate-x-0` memastikan sidebar selalu terlihat di desktop.
    - Overlay ditambahkan untuk menutup sidebar saat area luar diklik di mobile.
-->
...
<aside
    ...
    class="fixed inset-y-0 left-0 z-50 flex flex-col flex-shrink-0 w-64 bg-base-100 border-r border-base-300/50 transform transition-transform duration-300 ease-in-out lg:relative lg:translate-x-0"
    :style="isSidebarExpanded ? '' : 'transition: width 0.3s;'"
...
>
    ...
    <div class="p-2 border-t border-base-300/50">
        <div class="dropdown dropdown-top w-full">
            <label tabindex="0" class="btn btn-ghost w-full flex items-center overflow-hidden" :class="isSidebarExpanded ? 'justify-start' : 'justify-center'">
                <div class="avatar flex-shrink-0">
                    <div class="w-8 rounded-full">
                        <img src="https://ui-avatars.com/api/?name={{ urlencode(Auth::user()->name) }}&background=3B82F6&color=fff" alt="User Avatar" />
                    </div>
                </div>
                <div class="text-left ml-3 flex-1 min-w-0" x-show="isSidebarExpanded" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0">
