...
>
  ...
  <div class="flex h-20 shrink-0 items-center border-b border-base-300/50 px-4
    ...
    <a href="{{ route('dashboard') }}" class="flex items-center gap-3" @click="if (window.innerWidth < 1024) isMobileMenuOpen=false">
      ...
      <span class="origin-left whitespace-nowrap text-lg font-bold text-base-content hidden lg:group-hover/sidebar:inline-block">
        MatikGrowth
      </span>
    </a>
  </div>
  ...
  <nav class="flex-1 overflow-y-auto">
    ...
    <ul class="menu p-4 space-y-1">
      @foreach ($navGroups as $group => $links)
        {{-- Tampilkan judul grup hanya saat expanded (desktop hover) / mobile terbuka --}}
        <li class="menu-title hidden px-4 lg:group-hover/sidebar:block">
          <span>{{ $group }}</span>
        </li>
