...
class LeadController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // ⛏️ FIX: konsisten gunakan tabel 'wa_templates'
        $whatsappTemplates = Schema::hasTable('wa_templates')
            ? WATemplate::orderBy('name')->get(['id', 'name', 'body'])
            : collect();
    ...
    public function store(LeadRequest $request)
    {
        ...
        DB::transaction(function () use ($data, $registeredAt, $actor) {
            ...
            activity()->performedOn($lead)->causedBy($actor)->log("Membuat lead: {$lead->name}");

            // 📣 Notifikasi
            $actor?->notify(new GenericDbNotification(
                'Lead Baru',
                "Lead {$lead->name} berhasil dibuat.",
                route('leads.show', $lead)
            ));
            if ($lead->owner && $lead->owner_id !== $actor?->id) {
                $lead->owner->notify(new GenericDbNotification(
                    'Lead Baru untuk Anda',
                    "Anda ditetapkan sebagai owner untuk lead {$lead->name}.",
                    route('leads.show', $lead)
                ));
            }
        });
    ...
    public function update(LeadRequest $request, Lead $lead)
    {
        ...
        DB::transaction(function () use ($request, $lead, $data, $registeredAt, $actor) {
            $lead->update($data);
            ...
            activity()->performedOn($lead)->causedBy($actor)->log("Memperbarui lead: {$lead->name}");

            // 📣 Notifikasi
            $actor?->notify(new GenericDbNotification(
                'Lead Diperbarui',
                "Lead {$lead->name} berhasil diperbarui.",
                route('leads.show', $lead)
            ));
            if ($lead->owner && $lead->owner_id !== $actor?->id) {
                $lead->owner->notify(new GenericDbNotification(
                    'Lead Anda Diperbarui',
                    "Lead {$lead->name} telah diperbarui.",
                    route('leads.show', $lead)
                ));
            }
        });
    ...
    public function destroy(Lead $lead)
    {
        ...
        DB::transaction(function () use ($lead, $actor, $leadName) {
            ...
            activity()->causedBy($actor)->log("Menghapus lead: {$leadName}");
        });

        // 📣 Notifikasi
        $actor?->notify(new GenericDbNotification(
            'Lead Dihapus',
            "Lead {$leadName} telah dihapus.",
            route('leads.index')
        ));
    ...
    /** Tandai lead sudah di-chat secara manual */
    public function markChatted(Lead $lead)
    {
        ...
        // 📣 optional: notifikasi info
        $actor?->notify(new GenericDbNotification(
            'Chat Manual',
            "Anda menandai chat manual untuk {$lead->name}.",
            route('leads.show', $lead)
        ));
    ...
    public function sendWaToLead(Request $request, Lead $lead)
    {
        ...
        try {
            ...
            $actor?->notify(new GenericDbNotification(
                ...
                $ok
                    ? "Pesan ke {$lead->name} berhasil dikirim."
                    : ("Gagal mengirim pesan ke {$lead->name}".(isset($resp['error']) ? " ({$resp['error']})" : "")),
                route('leads.show', $lead)
            ));
            ...
            $actor?->notify(new GenericDbNotification(
                'WA Gagal',
                "Terjadi kesalahan saat mengirim pesan ke {$lead->name}.",
                route('leads.show', $lead)
            ));
    ...
    public function sendWaBulk(Request $request)
    {
        ...
        $actor?->notify(new GenericDbNotification(
            'Bulk WA Selesai',
            "Terkirim: {$sent}, Gagal: {$fail}.",
            route('leads.index')
        ));

        return response()->json(['success' => true, 'sent' => $sent, 'failed' => $fail]);
    }
}
