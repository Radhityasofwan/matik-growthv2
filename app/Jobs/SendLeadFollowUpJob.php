...
class SendLeadFollowUpJob implements ShouldQueue
{
    ...
    $resultLead = null; // <-- dipindah ke luar try agar bisa dipakai di catch
    ...
    try {
        ...
        if (!$to) {
            ...
            // NEW: tetap beritahu owner alasan skip
            $this->notifyOwner(
                $rule,
                $lead,
                $sender,
                '(SKIP: tanpa nomor lead)',
                ['status' => 'SKIPPED_NO_PHONE', 'http' => null]
            );
        ...
        activity('follow_up')->performedOn($lead)->withProperties([
            ...
            'status'    => data_get($resultLead, 'status') ?? 'UNKNOWN',
            'text'      => Str::limit($messageToLead, 500),
        ])->log('WA follow-up terkirim');

        $sent++;

        // success/fail (HTTP) tetap beritahu owner
        ...
        // NEW: exception path – tetap notifikasi owner
        $this->notifyOwner(
            $rule,
            $lead,
            $sender,
            '(ERROR saat kirim ke lead)',
            $resultLead ?? ['status' => 'EXCEPTION', 'http' => null, 'error' => $e->getMessage()]
        );
    }
}
    ...
    protected function notifyOwner(LeadFollowUpRule $rule, Lead $lead, WahaSender $sender, string $messageToLead, array $resultLead): void
    {
        ...
        $statusLead = data_get($resultLead, 'status') ?? 'UNKNOWN';
        ...
        try {
            ...
            activity('follow_up_notify_owner')->performedOn($lead)->causedBy($owner)->withProperties([
                ...
                'status'    => data_get($resultOwner, 'status'),
    ...
    protected function eligibleLeadsFor(LeadFollowUpRule $rule)
    {
        ...
        // SAMAKAN dengan Command: tanpa startOfDay
        $after = now()->copy()->subDays($days);
        ...
        $sub = Lead::query()
            ...
            ->selectSub(function ($q) {
                $q->from('activity_log')
                  ...
                  ->whereColumn('activity_log.subject_id', 'leads.id')
                  ->where('activity_log.subject_type', Lead::class)
                  // Konsisten dengan webhook: inbound = lead_reply
                  ->whereIn('activity_log.log_name', ['lead_reply']);
            }, 'meta_last_reply_at');
        ...
        switch ($rule->condition) {
            ...
            case 'chat_1_no_reply':
                $rows->where('meta_chat_count', '=', 1)
                     ...
                     ->where('meta_last_wa_chat_at', '<=', $after);
                break;
    ...
    protected function sendViaWaha(int $senderId, string $recipient, string $message): array
    {
        ...
        $resp = app()->call([$controller, 'sendMessage'], ['request' => $req]);
